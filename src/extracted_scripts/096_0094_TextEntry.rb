...
class Window_TextEntry_Keyboard < Window_TextEntry
  def initialize(text, x, y, width, height, heading = nil, usedarkercolor = false, regex_check = nil, max_length = nil)
    super(text, x, y, width, height, heading, usedarkercolor)
    @regex_check = regex_check
    
    # Set max length if specified
    @helper.maxlength = max_length if max_length
  end
  
  def insert(ch)
    # Check if character matches regex before inserting
    if @regex_check && !ch.match?(@regex_check)
      return false
    end
    super(ch)
  end  
  ...
  def update
    ...
    if Input.triggerex?(:LEFT) || Input.repeatex?(:LEFT)
      ...
      self.delete if @helper.cursor > 0
      return
    elsif (Input.triggerex?(:RETURN) || Input.triggerex?(:ESCAPE)) && (!Input.triggerex?(0x30) || !Input.repeatex?(0x30)) # 0x30 es el codigo del 0 en el numpad
      return
    end
