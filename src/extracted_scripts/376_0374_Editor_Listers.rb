...
#      Dir.glob("*.bmp") { |f| @commands.push(f) }
    ...
    pbMessage(_INTL("No hay archivos.")) if @commands.length == 0
    return @commands
  end
...
class MetadataLister
  ...
  def commands
    @commands.clear
    @commands.push(_INTL("[METADATA GLOBAL]"))
    @player_ids.each { |id| @commands.push(_INTL("Jugador {1}", id)) }
    @commands.push(_INTL("[AÑADIR NUEVO JUGADOR]")) if @new_player
    return @commands
  end
...
class SpeciesLister
  ...
  def commands
    ...
    if @includeNew
      @commands.push(_INTL("[NUEVA ESPECIE]"))
      @ids.push(true)
    end
    cmds.each do |i|
...
class ItemLister
  ...
  def commands
    ...
    cmds.sort! { |a, b| a[2].downcase <=> b[2].downcase }
    if @includeNew
      @commands.push(_INTL("[NUEVO OBJETO]"))
      @ids.push(true)
    end
    cmds.each do |i|
      @commands.push(sprintf("%03d: %s", i[0], i[2]))
...
class TrainerTypeLister
  ...
  def commands
    ...
    if @includeNew
      @commands.push(_INTL("[NUEVO TIPO DE ENTRENADOR]"))
      @ids.push(true)
    end
    cmds.each do |t|
...
class TrainerBattleLister
  ...
  def commands
    ...
    if @includeNew
      @commands.push(_INTL("[NUEVA BATALLA DE ENTRENADOR]"))
      @ids.push(true)
    end
    cmds.each do |t|
      if t[3] > 0
    ...
    if !@includeNew || index > 0
      ...
      if tr_data
        tr_data.pokemon.each_with_index do |pkmn, i|
          text += "\n" if i > 0
          text += sprintf("%s Nv.%d", GameData::Species.get(pkmn[:species]).real_name, pkmn[:level])
        end
      end
    end
    @pkmnList.text = text
