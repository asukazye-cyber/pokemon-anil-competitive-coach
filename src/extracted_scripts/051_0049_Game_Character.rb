class Game_Character
  ...
  def move_type_custom
    ...
    # ⚠️ SEM ROTA NAO SE ANDA — E NAO SE REBENTA.
    #
    # Um evento com `move_type == 3` (rota personalizada) e `@move_route` a nil
    # mata o jogo a cada frame:
    #
    #     undefined method `list' for nil:NilClass   (0049_Game_Character:429)
    #
    # Isso acontece com eventos que vem de um save antigo, ou que foram
    # injectados em codigo sem rota — os itens largados, os Pokemon soltos, o
    # lendario do evento. O motor nunca previu um evento sem rota porque no
    # editor isso nao existe; em codigo existe.
    #
    # Voltar atras aqui deixa o evento parado, que e exactamente o que ele ja
    # estava a fazer. Nao se perde comportamento nenhum, e deixa de se perder a
    # partida. O `@original_move_route` leva a mesma rede, porque a restauracao
    # la em baixo pode devolver nil pelo mesmo motivo.
    return if @move_route.nil? || @move_route.list.nil?
