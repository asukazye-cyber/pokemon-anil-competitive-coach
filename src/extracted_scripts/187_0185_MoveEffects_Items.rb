...
class Battle::Move::UserTargetSwapItems < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    user.effects[PBEffects::ChoiceBand]   = nil if !user.hasActiveAbility?(:GORILLATACTICS) && !user.hasActiveAbility?(:PODERSABIO)
    user.effects[PBEffects::Unburden]     = (!user.item && oldUserItem) if user.hasActiveAbility?(:UNBURDEN)
    target.item                           = oldUserItem
    target.effects[PBEffects::ChoiceBand] = nil if !target.hasActiveAbility?(:GORILLATACTICS) && !target.hasActiveAbility?(:PODERSABIO)
