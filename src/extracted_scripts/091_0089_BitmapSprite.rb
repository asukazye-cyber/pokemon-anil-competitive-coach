...
class ChangelingSprite < Sprite
  ...
  def add_bitmap(mode, *data)
    ...
    # ⚠️ REGISTA O CAMINHO, NAO CARREGA A IMAGEM.
    #
    # Quem chama isto costuma declarar MUITO mais do que chega a mostrar. O caso
    # que se mediu: o plugin Enhanced Pokemon UI percorre todas as pokebolas do
    # jogo e declara duas imagens por cada uma em CADA painel da party — 28
    # imagens por painel, seis paineis, para mostrar uma bola em cada.
    #
    # Isso custava duas vezes: 168 ficheiros lidos do disco ao abrir a party, e
    # depois 168 AnimatedBitmap#update por frame enquanto ela estivesse aberta
    # (ver o update aqui em baixo, que percorre @bitmaps todo).
    #
    # Agora so se guarda o caminho. Quem carrega e o change_bitmap, a primeira
    # vez que a imagem for mesmo precisa. Para quem chama nao muda nada.
    @bitmaps[filepath] = nil if !@bitmaps.key?(filepath)
  ...
  def change_bitmap(mode)
    ...
    if @current_mode && @changeling_data[@current_mode]
      ...
      # e aqui, so quando esta imagem e mesmo para aparecer, e que ela e lida
      @bitmaps[data[0]] ||= AnimatedBitmap.new(data[0])
  ...
  def dispose
    return if disposed?
    # o `if bm` e por causa dos caminhos declarados que nunca chegaram a ser
    # carregados: ficam a nil no @bitmaps
    @bitmaps.each_value { |bm| bm.dispose if bm }
    @bitmaps.clear
    super
  end
  ...
  def update
    return if disposed?
    @bitmaps.each_value { |bm| bm.update if bm }
