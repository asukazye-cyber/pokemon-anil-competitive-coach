module Compiler
    ...
    paths.each do |path|
      ...
      pbCompilerEachPreppedLine(path) do |line, lineno|
        ...
        if !pbRgssExists?(sprintf("Data/Map%03d.rxdata", record[0]))
          print _INTL("Advertencia: El mapa {1}, mencionado en los datos de conexión de mapas, no se encontró.\n{2}", record[0], FileLineData.linereport)
        elsif !pbRgssExists?(sprintf("Data/Map%03d.rxdata", record[3]))
          print _INTL("Advertencia: El mapa {1}, mencionado en los datos de conexión de mapas, no se encontró.\n{2}", record[3], FileLineData.linereport)
        end
        case record[1]
        when "N"
          raise _INTL("El lado norte del primer mapa debe conectarse con el lado sur del segundo mapa\n{1}", FileLineData.linereport) if record[4] != "S"
        when "S"
          raise _INTL("El lado sur del primer mapa debe conectarse con el lado norte del segundo mapa\n{1}", FileLineData.linereport) if record[4] != "N"
        when "E"
          raise _INTL("El lado este del primer mapa debe conectarse con el lado oeste del segundo mapa\n{1}", FileLineData.linereport) if record[4] != "W"
        when "W"
          raise _INTL("El lado oeste del primer mapa debe conectarse con el lado este del segundo mapa\n{1}", FileLineData.linereport) if record[4] != "E"
        end
        records.push(record)
      end
      process_pbs_file_message_end
  ...
  def validate_all_compiled_types
    ...
    GameData::Type.each do |type|
      ...
      type.weaknesses.each do |other_type|
        ...
        raise _INTL("'{1}' no es un tipo definido ({2}, sección {3}, Debilidades).", other_type.to_s, path, type.id)
      end
      type.resistances.each do |other_type|
        next if GameData::Type.exists?(other_type)
        raise _INTL("'{1}' no es un tipo definido ({2}, sección {3}, Resistencias).", other_type.to_s, path, type.id)
      end
      type.immunities.each do |other_type|
        next if GameData::Type.exists?(other_type)
        raise _INTL("'{1}' no es un tipo definido ({2}, sección {3}, Inmunidades).", other_type.to_s, path, type.id)
      end
  ...
  def validate_compiled_move(hash)
    if (hash[:category] || 2) == 2 && (hash[:power] || 0) != 0
      raise _INTL("El movimiento {1} está definido como un movimiento de Estado con un daño base distinto de cero.\n{2}",
                  hash[:real_name], FileLineData.linereport)
    elsif (hash[:category] || 2) != 2 && (hash[:power] || 0) == 0
      print _INTL("Advertencia: El movimiento {1} está definido como Físico o Especial pero tiene un daño base de 0. Se cambiará a un movimiento de Estado.\n{2}",
                  hash[:real_name], FileLineData.linereport)
      hash[:category] = 2
    end
  end
  ...
  def compile_regional_dexes(*paths)
    ...
    dex_lists.each_with_index do |list, index|
      ...
      list.each_with_index do |s, i|
        ...
        raise _INTL("El número de la lista de la Pokédex {1} tiene la especie {2} listada dos veces.\n{3}", index, s, FileLineData.linereport)
      end
    end
    # Save all data
      ...
      pbCompilerEachPreppedLine(path) do |line, line_no|
        ...
        if current_type && line[/^\d+,/]   # Species line
          ...
          if !values || values.length < 3
            raise _INTL("Se esperaba una línea de entrada de especies para el tipo de encuentro {1} en el mapa '{2}', se obtuvo \"{3}\" en su lugar.\n{4}",
          ...
          if values[2] > max_level
            raise _INTL("El número de nivel {1} no es válido (máx. {2}).\n{3}", values[2], max_level, FileLineData.linereport)
          ...
          raise _INTL("Se esperaba un número de mapa, se recibió \"{1}\" en su lugar.\n{2}", line, FileLineData.linereport)
        else
          ...
          if current_type && GameData::EncounterType.exists?(current_type)   # Start of a new encounter method
            ...
            raise _INTL("Tipo de encuentro \"{1}\" indefinido para el mapa '{2}'.\n{3}",
      ...
      pbCompilerEachPreppedLine(path) do |line, line_no|
        ...
        if line[/^\s*\[\s*(.+)\s*\]\s*$/]
          ...
          if schema[key]   # Property of the trainer
            ...
            if !current_pkmn
              raise _INTL("¡El Pokémon aún no ha sido definido!\n{1}", FileLineData.linereport)
            end
  ...
  def validate_compiled_trainer(hash)
    ...
    if hash[:pokemon].empty?
      raise _INTL("El entrenador con ID {1} no tiene Pokémon.\n{2}", hash[:id], FileLineData.linereport)
    end
    max_level = GameData::GrowthRate.max_level
    hash[:pokemon].each do |pkmn|
      # Ensure valid level
      if pkmn[:level] > max_level
        raise _INTL("Nivel de Pokémon no válido {1} (debe ser de 1 a {2}).\n{3}",
      ...
      if pkmn[:real_name] && pkmn[:real_name].length > Pokemon::MAX_NAME_SIZE
        raise _INTL("Apodo de Pokémon no válido: {1} (debe tener de 1 a {2} caracteres).\n{3}",
      ...
      if pkmn[:iv]
        ...
        GameData::Stat.each_main do |s|
          ...
          if iv_hash[s.id] > Pokemon::IV_STAT_LIMIT
            raise _INTL("IV no válido: {1} (debe estar entre 0 y {2}).\n{3}",
      ...
      if pkmn[:ev]
        ...
        GameData::Stat.each_main do |s|
          ...
          if ev_hash[s.id] > Pokemon::EV_STAT_LIMIT
            raise _INTL("EV no válido: {1} (debe estar entre 0 y {2}).\n{3}",
                        ev_hash[s.id], Pokemon::EV_STAT_LIMIT, FileLineData.linereport)
          end
        end
        pkmn[:ev] = ev_hash
        if ev_total > Pokemon::EV_LIMIT
          raise _INTL("Conjunto de EV no válido (debe sumar {1} o menos).\n{2}",
      ...
      if pkmn[:poke_ball]
        if !GameData::Item.get(pkmn[:poke_ball]).is_poke_ball?
          raise _INTL("El valor {1} no es una Poké Ball definida.\n{2}", pkmn[:poke_ball], FileLineData.linereport)
        end
      end
    end
  end
  ...
  def compile_trainer_lists(path = "PBS/battle_facility_lists.txt")
    ...
    File.open(path, "rb") do |f|
      ...
      pbEachFileSection(f) do |section, name|
        ...
        if !rsection[0]
          raise _INTL("No se proporcionó el archivo de datos del entrenador en la sección {1}.\n{2}", name, FileLineData.linereport)
        end
        if !rsection[1]
          raise _INTL("No se proporcionó el archivo de datos del entrenador en la sección {1}.\n{2}", name, FileLineData.linereport)
        end
  ...
  def validate_compiled_global_metadata(hash)
    if hash[:home].nil?
      raise _INTL("La entrada 'Home' es requerida en la sección 0 de metadata.txt.\n{1}", FileLineData.linereport)
    end
  end
  ...
  def validate_all_compiled_metadata
    ...
    if !GameData::Metadata.exists?(0)
      raise _INTL("El metadata global no está definido en metadata.txt pero debería estarlo.\n{1}", FileLineData.linereport)
    end
    # Ensure player character 1's metadata is defined
    if !GameData::PlayerMetadata.exists?(1)
      raise _INTL("El metadata para el personaje del jugador 1 no está definido en metadata.txt pero debería estarlo.\n{1}", FileLineData.linereport)
    end
