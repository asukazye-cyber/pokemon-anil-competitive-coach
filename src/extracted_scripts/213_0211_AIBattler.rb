...
class Battle::AI::AIBattler
  ...
  def rough_end_of_round_damage
    ...
    if self.effects[PBEffects::AquaRing] && battler.canHeal?
      ...
      amt = (amt * 1.3).floor if has_active_ability?(:STURDYROOT)
      ret -= [amt, 1].max
    end
    # Ingrain
      ...
      battler.pbTypes(true).each do |defend_type|
        ...
        if move
          ...
          when "SuperEffectiveAgainstBug"
            mult = Effectiveness::SUPER_EFFECTIVE_MULTIPLIER if defend_type == :BUG
          end
        end
        ret *= mult
      end
  ...
  def wants_status_problem?(new_status)
    ...
    if ability_active?
      case ability_id
      when :GUTS
        return true if ![:SLEEP, :FROZEN, :FROSTBITE].include?(new_status) &&
