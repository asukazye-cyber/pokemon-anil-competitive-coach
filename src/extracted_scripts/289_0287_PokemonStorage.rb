...
class PokemonStorage
  ...
  def maxPokemon(box)
    ...
    return (box < 0) ? max_party_size : self[box].length #Settings::MAX_PARTY_SIZE : self[box].length
  end
  ...
  def pbFindBoxWithSpace(order = :asc)
    if order == :asc
      (0...self.maxBoxes).each do |i|
        return i if self.pbFirstFreePos(i) >= 0
      end
    else
      (self.maxBoxes-1).downto(0) do |i|
        return i if self.pbFirstFreePos(i) >= 0
      end
    end
    return -1
...
class RegionalStorage
  ...
  def pbFindBoxWithSpace(order = :asc)
    getCurrentStorage.pbFindBoxWithSpace(order)
  end
