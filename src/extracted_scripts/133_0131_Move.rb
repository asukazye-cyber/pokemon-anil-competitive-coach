module GameData
  class Move
    ...
    def initialize(hash)
      @id               = hash[:id]
      @real_name        = hash[:real_name]        || "Sin nombre"
      @type             = hash[:type]             || :NONE
      ...
      @function_code    = hash[:function_code]    || "Ninguno"
      @flags            = hash[:flags]            || []
      ...
      when "ThrowUserItemAtTarget"
        fling_powers = {
          ...
          100 => [:HARDSTONE,:RAREBONE,
           ...
           40 => [:EVIOLITE,:ICYROCK,:LUCKYPUNCH, :SUPEREVIOLITE
                 ],
           30 => [:ABSORBBULB,:ADRENALINEORB,:AMULETCOIN,:BINDINGBAND,:BLACKBELT,
                  ...
                  :SUPERCALCIUM,:SUPERCARBOS,:SUPERHPUP,:SUPERIRON,:SUPERPROTEIN,:SUPERZINC,
                  :RARECANDY,
    ...
    def display_power(pkmn, move = nil)
      if Settings::SHOW_MODIFIED_MOVE_PROPERTIES
        ...
        when "DoublePowerIfUserHasNoItem"
          return @power * 2 if !pkmn.item || pkmn.item == :FLYINGGEM
          return @power
        when "ThrowUserItemAtTarget"
          item_data = pkmn.item
          if item_data
