...
class Battle::AI
  ...
  BASE_ABILITY_RATINGS = {
    ...
    8  => [:ADAPTABILITY, :AERILATE, :CONTRARY, :DISGUISE, :DRAGONSMAW,
           ...
           :PIELMALDITA, :PIELHERBACEA, :REALEZA],
    7  => [:BEASTBOOST, :BULLETPROOF, :COMPOUNDEYES, :DOWNLOAD, :FURCOAT,
           ...
           :VOLTABSORB, :WATERABSORB, :ESPANTO],
    6  => [:BATTLEBOND, :CHLOROPHYLL, :COMATOSE, :DARKAURA, :DRYSKIN,
           ...
           :TRACE, :UNAWARE, :VICTORYSTAR, :CAMORRISTA, :TINTINEO],
    5  => [:AFTERMATH, :AIRLOCK, :ANALYTIC, :BERSERK, :BLAZE, :CLOUDNINE,
           ...
           :COLEOPTERO, :INFLAMABLE, :ALBINISMO,
           :PRESSURE, :QUICKFEET, :ROCKHEAD, :SANDSPIT, :SHIELDDUST, :SLUSHRUSH,
           :SWARM, :TANGLINGHAIR, :TORRENT,:SOBRECARGA, :ICEHEAD, :PODERGELIDO],
    4  => [:ANGERPOINT, :BADDREAMS, :CHEEKPOUCH, :CLEARBODY, :CURSEDBODY,
           ...
           :WHITESMOKE, :WONDERSKIN, :PODERSABIO, :ACOMETIDA, :SILVANO],
  ...
  BASE_ITEM_RATINGS = {
    10 => [:EVIOLITE, :FOCUSSASH, :LIFEORB, :THICKCLUB, :SUPEREVIOLITE],
...
Battle::AI::Handlers::AbilityRanking.add(:CAMORRISTA,
  proc { |ability, score, battler, ai|
...
Battle::AI::Handlers::AbilityRanking.add(:TINTINEO,
  proc { |ability, score, battler, ai|
    next score if battler.check_for_move { |m| m.soundMove? }
    next 0
  }
...
Battle::AI::Handlers::AbilityRanking.add(:SOBRECARGA,
  proc { |ability, score, battler, ai|
...
Battle::AI::Handlers::AbilityRanking.add(:COLEOPTERO,
  proc { |ability, score, battler, ai|
...
Battle::AI::Handlers::AbilityRanking.add(:INFLAMABLE,
  proc { |ability, score, battler, ai|
    next score if battler.has_damaging_move_of_type?(:FIRE)
    next 0
  }
...
Battle::AI::Handlers::AbilityRanking.add(:ALBINISMO,
  proc { |ability, score, battler, ai|
...
Battle::AI::Handlers::AbilityRanking.copy(:ROCKHEAD, :ICEHEAD)
...
Battle::AI::Handlers::AbilityRanking.add(:PODERGELIDO,
  proc { |ability, score, battler, ai|
    next score if battler.has_damaging_move_of_type?(:ICE)
    next 0
  }
...
Battle::AI::Handlers::AbilityRanking.add(:FLORACION,
  proc { |ability, score, battler, ai|
    next score if battler.has_damaging_move_of_type?(:GRASS)
    next 0
  }
...
Battle::AI::Handlers::AbilityRanking.add(:REALEZA,
  proc { |ability, score, battler, ai|
    next score if battler.has_damaging_move_of_type?(GameData::Type.keys)
    next 0
  }
)
...
Battle::AI::Handlers::ItemRanking.add(:SUPEREVIOLITE,
  proc { |item, score, battler, ai|
    next score if battler.battler.pokemon.species_data.get_evolutions(true).length > 1
    next 0
  }
)
...
Battle::AI::Handlers::ItemRanking.add(:LIGHTBALL,
  proc { |item, score, battler, ai|
    next score if ( battler.battler.isSpecies?(:PIKACHU) || battler.battler.isSpecies?(:RAICHU)) &&
                  battler.check_for_move { |m| m.damagingMove? }
    next 0
  }
)
