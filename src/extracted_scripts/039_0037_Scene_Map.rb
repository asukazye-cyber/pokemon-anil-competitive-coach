...
class Scene_Map
  ...
  def dispose
    disposeSpritesets
    @map_renderer.dispose if @map_renderer
    @map_renderer = nil
  ...
  def autofade(mapid)
    ...
    begin
      ...
      raise "Error loading Map #{mapid}: #{e.message}"
    end
  ...
  def transfer_player(cancel_swimming = true)
    ...
    pbBridgeOff
    @spritesetGlobal.playersprite.clearShadows if @spritesetGlobal && @spritesetGlobal.playersprite
    if $game_map.map_id != $game_temp.player_new_map_id
      $map_factory.setup($game_temp.player_new_map_id)
    end
    # Reset/apply weather instantly based on the target map type to prevent 1-frame weather leaks/flickers
    if $game_screen
      metadata = GameData::MapMetadata.try_get($game_map.map_id)
      if !metadata || !metadata.outdoor_map
        $game_screen.weather(:None, 0, 0)
        # Stop overworld weather BGS instantly
        playing_bgs = $game_system.playing_bgs
        if playing_bgs && ["Rain", "Storm", "Sandstorm", "Wind"].include?(playing_bgs.name)
          pbBGSStop(0.0) rescue nil
        end
      else
        expected = :None
        if defined?(AnilLanRework)
          expected = AnilLanRework.get_weather_for_map($game_map.map_id) rescue :None
        end
        target_weather = (expected == :Moonlight) ? :None : expected
        $game_screen.weather(target_weather, 9, 0)
      end
    end
    ...
    $game_map.update
    disposeSpritesets
    # RPG::Cache.clear - Removed to avoid massive reload lag on every map transfer
    createSpritesets
  ...
  def update
    ...
    # input_blocked? = chat aberto OU janela de carência pós-fechamento. A
    # carência é essencial: o Enter que envia a mensagem também é o Input::USE,
    # e sem ela o mesmo Enter, no frame do fechamento, cairia no bloco abaixo e
    # (a) interagiria com o NPC e (b) reabriria o chat via triggerex?(:RETURN).
    return if defined?(AnilLanRework::ChatInputHUD) && AnilLanRework::ChatInputHUD.respond_to?(:input_blocked?) && AnilLanRework::ChatInputHUD.input_blocked?
    if !pbMapInterpreterRunning? && !$PokemonGlobal.forced_movement?
      ...
      # ⚠️ O T ja NAO abre o chat na descida da tecla.
      #
      # Segurar o T mostra o historico das mensagens (MOD 182), e um toque abre
      # o chat. Para distinguir os dois, a decisao tem de ser tomada quando a
      # tecla e LARGADA — se abrisse na descida, o chat ja estava aberto antes
      # de se perceber que o jogador estava a segurar.
      #
      # Quem tiver o MOD desligado ou em falta cai no comportamento antigo.
      # O ENTER continua a abrir na descida, sem espera nenhuma.
      elsif (defined?(AnilChatHistorico) ? (AnilChatHistorico.pedir_abrir_chat? rescue false)
                                         : (Input.triggerex?(:T) rescue false)) ||
            (Input.triggerex?(:RETURN) rescue false)
        if defined?(AnilLanRework) && AnilLanRework.connected? && !$game_player.moving? && !$game_temp.menu_calling
          if defined?(AnilLanRework::ChatInputHUD)
            AnilLanRework::ChatInputHUD.open
          else
            message = pbEnterText(_INTL("Mensagem de Chat (Online):"), 0, 80, "")
            if !message.to_s.strip.empty?
              AnilLanRework::Chat.send_message(message)
            end
          end
        end
      end
