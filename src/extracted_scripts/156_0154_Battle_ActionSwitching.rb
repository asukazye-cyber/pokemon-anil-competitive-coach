  ...
  def pbCanSwitchOut?(idxBattler, partyScene = nil)
    ...
    allOtherSideBattlers(idxBattler).each do |b|
      ...
      if Battle::ItemEffects.triggerTrappingByTarget(b.item, battler, b, self)
        partyScene&.pbDisplay(_INTL("¡{1} de {2} previene el cambio!", b.itemName, b.pbThis))
        return false
      end
    end
    return true
      ...
      @battlers.each do |b|
        ...
        if !pbOwnedByPlayer?(idxBattler)   # Opponent/ally is switching in
          ...
          if @internalBattle && @switchStyle && trainerBattle? && pbSideSize(0) == 1 &&
            ...
            # Logica para que no te diga a que Pokémon va a cambiar
            $PokemonSystem.show_pokemon_on_change ||= 0
            change_message = $PokemonSystem.show_pokemon_on_change == 0 ? _INTL("{1} va a sacar a {2}. ¿Quieres cambiar de Pokémon?",
                                      opponent.full_name, enemyParty[idxPartyForName].name) : _INTL("{1} cambiará de Pokémon. ¿Quieres cambiar tu también?",
                                      opponent.full_name)
