class Sprite_SurfBase
  ...
  def update
    return if disposed?
    # ⚠️ MONTADO NAO HA JANGADA.
    #
    # Montado, o veiculo e a montaria. Sem isto, entrar na agua desenhava a
    # jangada do surf POR BAIXO do Pokemon — dois veiculos ao mesmo tempo. O
    # sprite trata-se aqui como se nao houvesse surf nenhum, que e o ramo que
    # ja existia e que se limpa sozinho.
    montado = (defined?(AnilMontaria) && AnilMontaria.montado?) rescue false
    if montado || (!$PokemonGlobal.surfing && !$PokemonGlobal.diving)
