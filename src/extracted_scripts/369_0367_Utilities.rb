...
def getConstantName(mod, value, raise_if_none = true)
  ...
  raise _INTL("Valor {1} no definido por una constante en {2}", value, mod.name) if raise_if_none
  return nil
end
...
def pbNoticePlayer2(event, always_show_exclaim = false)
  if always_show_exclaim || !pbFacingEachOther(event, $game_player)
    pbExclaim(event)
  end
  pbTurnTowardEvent($game_player, event)
...
def pbTrainerName(name = nil, outfit = 0)
  ...
  if name.nil?
    name = pbEnterPlayerName(_INTL("¿Tu nombre?"), 0, Settings::MAX_PLAYER_NAME_SIZE)
...
def pbMoveTutorChoose(move, movelist = nil, bymachine = false, oneusemachine = false)
  ...
  pbFadeOutIn do
    ...
    screen.pbStartScene(_INTL("¿Enseñar a qué Pokémon?"), false, annot)
    loop do
      ...
      if pokemon.egg?
        ...
        pbMessage(_INTL("Los Pokémon Oscuros no pueden aprender ningún movimiento.")) { screen.pbUpdate }
      elsif movelist && movelist.none? { |j| j == pokemon.species }
        pbMessage(_INTL("{1} no puede aprender {2}.", pokemon.name, movename)) { screen.pbUpdate }
...
def pbScreenCapture
  ...
  pbSEPlay("Camara de fotos") if FileTest.audio_exist?("Audio/SE/Camara de fotos")
end
