...
#    6 => [["Water rock", "Sea deep"], []]
  ...
  class TilesetBitmaps
    ...
    # ⚠️ O TILESET E DEITADO FORA E VOLTA A SER LIDO A CADA MUDANCA DE MAPA.
    #
    # Um tileset destes e uma tira alta — o Exterior1 tem 256x23370, ou seja
    # 22,8 MB depois de descodificado. E como e "mega", ainda leva um
    # TilesetWrapper.wrapTileset por cima, que refaz a imagem inteira.
    #
    # Tudo isso e feito ao entrar no mapa e desfeito ao sair, mesmo quando se
    # volta ao mesmo sitio um segundo depois. Medido no JoiPlay: 50 ms por
    # transicao, e ate 101 ms quando ele e pedido duas vezes.
    #
    # Aqui guardam-se os ultimos, em vez de os libertar. E uma troca directa de
    # memoria por tempo, e por isso tem tecto: RESERVA_MAX tilesets. A ordem de
    # insercao do Hash faz de lista — o mais antigo e o primeiro a sair.
    #
    # A reserva e da CLASSE, e nao da instancia: cada mapa cria o seu
    # TilemapRenderer, portanto uma reserva por instancia morria com o mapa e
    # nao servia de nada.
    RESERVA_MAX = 2
    ...
    # Guarda em vez de libertar. Se ja nao couber, sai o mais antigo.
    def self.guardar(filename, bitmap, wrap)
      return if filename.nil? || bitmap.nil?
      return bitmap.dispose if (bitmap.disposed? rescue true)
      if reserva.key?(filename)
        reserva.delete(filename)   # volta a entrar como o mais recente
      end
      reserva[filename] = [bitmap, wrap]
      while reserva.length > RESERVA_MAX
        _velho, par = reserva.shift
        par[0].dispose if par && par[0] && !(par[0].disposed? rescue true)
      end
    rescue
      nil
    end

    def self.recuperar(filename)
      par = reserva.delete(filename)
      return nil unless par && par[0]
      return nil if (par[0].disposed? rescue true)
      par
    rescue
      nil
    end
    ...
    def add(filename)
      ...
      # ainda o temos guardado da ultima vez? entao nao ha nada a ler
      if (guardado = TilesetBitmaps.recuperar(filename))
        self[filename] = guardado[0]
        @bitmap_wraps[filename] = guardado[1]
        @load_counts[filename] = 1
        return
      end
    ...
    def remove(filename)
      ...
      # em vez de libertar, guarda-se para a proxima entrada neste mapa
      preservar(filename, @bitmaps[filename], @bitmap_wraps[filename])
      @bitmaps.delete(filename)
      @bitmap_wraps.delete(filename)
      @load_counts.delete(filename)
    end

    # ⚠️ SO OS TILESETS ENTRAM NA RESERVA.
    #
    # O AutotileBitmaps herda este `remove`, e por isso guardava tambem cada
    # autotile na MESMA reserva de dois lugares. O Spriteset_Map#dispose faz,
    # por esta ordem:
    #
    #     remove_tileset(...)            -> Exterior1 entra na reserva
    #     autotile_names.each { remove }  -> 2 a 7 autotiles entram a seguir
    #
    # Com RESERVA_MAX = 2, o Exterior1 era o primeiro a sair — e a sair quer
    # dizer `dispose`. A reserva existia, enchia-se de autotiles, e o tileset
    # de 22,8 MB era relido do disco em TODAS as viagens. Era exactamente o que
    # o log mostrava: `tileset:ler_do_disco` 18 vezes em 18 transicoes.
    #
    # Os autotiles nem beneficiavam disso: o `add` deles nunca consulta a
    # reserva (passa direito ao pbGetAutotile + AutotileExpander), portanto o
    # que la punham nunca era recuperado. Custo puro.
    def preservar(filename, bitmap, wrap)
      TilesetBitmaps.guardar(filename, bitmap, wrap)
    end
  ...
  class AutotileBitmaps < TilesetBitmaps
    ...
    # Um autotile e pequeno e barato de refazer. Nao vai para a reserva: o lugar
    # la e do tileset, que e a imagem cara. Volta ao comportamento de origem.
    def preservar(_filename, bitmap, _wrap)
      bitmap.dispose if bitmap && !(bitmap.disposed? rescue true)
    rescue
      nil
    end
  ...
  def dispose
    ...
    @tiles.clear
    # ⚠️ ESTE CAMINHO PASSA AO LADO DO remove, E TAMBEM TEM DE GUARDAR.
    #
    # Sem isto a reserva era inutil: o Spriteset_Map chama o remove_tileset, mas
    # quando o renderer inteiro e libertado os bitmaps morriam aqui na mesma.
    @tilesets.bitmaps.each_pair do |nome, bitmap|
      TilemapRenderer::TilesetBitmaps.guardar(nome, bitmap, @tilesets.instance_variable_get(:@bitmap_wraps)[nome])
    end
