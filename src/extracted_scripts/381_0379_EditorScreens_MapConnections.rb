...
class MapScreenScene
  ...
  def mapScreen
    ...
    @sprites["title"] = Window_UnformattedTextPokemon.newWithSize(
      _INTL("D: Ayuda"), 0, Graphics.height - 64, Graphics.width, 64, @viewport
    )
    @sprites["title"].z = 2
  ...
  def helpWindow
    helptext = _INTL("A: Agregar mapa al lienzo") + "\n"
    ...
    helptext += _INTL("Teclas de flecha/arrastrar lienzo: Moverse por el lienzo")
...
#   echoln "mousemove (#{mapid},#{x},#{y})"
    if @dragging
      if @dragmapid >= 0
        ...
        sprite.x = x & ~3
        sprite.y = y & ~3
        @sprites["title"].text = _ISPRINTF("D: Ayuda [{1:03d}: {2:s}]", mapid, @mapinfos[@dragmapid].name)
      else
        xpos = x - @dragOffsetX
        ypos = y - @dragOffsetY
        ...
        @sprites["title"].text = _INTL("D: Ayuda")
      end
    elsif mapid >= 0
      @sprites["title"].text = _ISPRINTF("D: Ayuda [{1:03d}: {2:s}]", mapid, @mapinfos[mapid].name)
    else
      @sprites["title"].text = _INTL("D: Ayuda")
    end
  end
  ...
  def update
    ...
    if Input.triggerex?(:A)
      id = chooseMapScreen(_INTL("Añadir Mapa"), @currentmap)
      if id > 0
        addSprite(id)
        setTopSprite(id)
      ...
      id = chooseMapScreen(_INTL("Ir a Mapa"), @currentmap)
      if id > 0
        @mapconns = generateConnectionData
        pbDisposeSpriteHash(@mapsprites)
        @mapsprites.clear
