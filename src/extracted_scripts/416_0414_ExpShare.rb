...
if Settings::USE_NEW_EXP_SHARE
    ...
    class PokemonPartyPanel < Sprite
        ...
        def refresh_overlay_information
            # ⚠️ O ICONE TEM DE FICAR DENTRO DA MESMA GUARDA QUE O RESTO.
            #
            # O metodo original comeca com `return if !@refreshBitmap`: o painel
            # so se redesenha quando alguma coisa mudou. Este envolvimento
            # chamava o original (que respeitava a guarda) e a seguir desenhava o
            # icone SEMPRE — inclusive nos frames em que nao havia nada a
            # redesenhar.
            #
            # E o desenho nao e barato: o pbDrawImagePositions vai buscar o
            # ficheiro ao disco a cada chamada. Medido: `expicon.png` carregado
            # 229 vezes para ABRIR a party e 228 para a FECHAR — ao fechar era o
            # unico ficheiro a ser lido, ou seja, todo aquele tempo era isto.
            #
            # Pior: como o overlay so e limpo dentro da guarda, o icone estava a
            # ser carimbado por cima de si proprio, frame apos frame. O mesmo
            # desenho, no mesmo sitio, centenas de vezes.
