...
class PokemonSummary_Scene
  ...
  def drawPageThree
    ...
    if ability
      ...
      drawTextEx(overlay, 228, 338, 282, 2, "[D]: Descripción", Color.new(64, 64, 64), Color.new(176, 176, 176))
    end
    # Draw all text
    pbDrawTextPositions(overlay, textpos)
    # Draw HP bar
  ...
  def drawSelectedMove(move_to_learn, selected_move)
    ...
    move_data = GameData::Move.get(selected_move.id)
    power = move_data.display_power(@pokemon)
    case power
    ...
    else        textpos.push([power.to_s, 216, 160, :right, base, shadow])
    end
