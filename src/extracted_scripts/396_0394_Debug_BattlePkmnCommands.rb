...
MenuHandlers.add(:battle_pokemon_debug_menu, :set_ability, {
  ...
  "effect" => proc { |pkmn, battler, battle|
    cmd = 0
    commands = []
    commands.push(_INTL("Definir la habilidad de tu Pokémon"))
    commands.push(_INTL("Definir la habilidad del rival")) if battler
    commands.push(_INTL("Resetear"))
    loop do
      if battler
        msg = _INTL("La habilidad del rival es {1}. La habilidad del Pokémon es {2}.",
                    battler.abilityName, pkmn.ability.name)
      else
        msg = _INTL("La habilidad de tu Pokémon es {1}.", pkmn.ability.name)
      end
      cmd = pbMessage("\\ts[]" + msg, commands, -1, nil, cmd)
      break if cmd < 0
