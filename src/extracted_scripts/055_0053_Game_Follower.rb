...
class Game_Follower < Game_Event
  ...
  def location_passable?(x, y, direction)
    ...
    this_map.events.each_value do |event|
      # Mesmo caso do Game_Map#passableStrict?: evento injetado por mod pode vir
      # com tile_id nil, e o "< 0" cru derruba o jogo com
      # "undefined method '<' for nil:NilClass" enquanto o follower anda.
      event_tile_id = (this_map.respond_to?(:anil_tile_id_de) ? this_map.anil_tile_id_de(event) : ((event.tile_id rescue nil) || 0))
      ocupa = if this_map.respond_to?(:anil_evento_ocupa?)
                this_map.anil_evento_ocupa?(event, x, y)
              else
                (event.at_coordinate?(x, y) rescue false)
              end
      next if event_tile_id < 0 || event.through || !ocupa
      tile_data = GameData::TerrainTag.try_get(this_map.terrain_tags[event_tile_id])
      next if tile_data.ignore_passability
      next if tile_data.bridge && $PokemonGlobal.bridge == 0
      return false if tile_data.ledge
      passage = this_map.passages[event_tile_id] || 0
      return false if passage & bit != 0
      passed_tile_checks = true if (tile_data.bridge && $PokemonGlobal.bridge > 0) ||
                                   (this_map.priorities[event_tile_id] || -1) == 0
