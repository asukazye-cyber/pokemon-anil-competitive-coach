...
class Pokemon
  ...
  def change_heart_gauge(method, multiplier = 1)
    ...
    when "scent"
      ...
      raise _INTL("Cambio de medidor de la puerta del corazón desconocido: {1}", method.to_s)
    end
    adjustHeart(-amt)
  end
  ...
  def check_ready_to_purify
    return if !shadowPokemon?
    update_shadow_moves
    pbMessage(_INTL("¡{1} ahora puede ser purificado!", self.name)) if self.heart_gauge == 0
  end
