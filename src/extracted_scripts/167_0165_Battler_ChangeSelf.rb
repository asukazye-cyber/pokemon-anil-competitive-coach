class Battle::Battler
  ...
  def pbReduceHP(amt, anim = true, registerDamage = true, anyAnim = true)
    ...
    # Mensaje de PS perdidos para debuguear
    # p "[HP LOSS] #{pbThis} pierde #{amt} PS (HP antes: #{oldHP}, HP ahora: #{@hp})"
    return amt
  end
  ...
  def pbRecoverHPFromDrain(amt, target, msg = nil)
    if target.hasActiveAbility?(:LIQUIDOOZE, true)
      ...
      if canHeal?
        ...
        amt = (amt * 1.3).floor if hasActiveAbility?(:STURDYROOT)
        pbRecoverHP(amt)
      end
    end
  end
  ...
  def pbFaint(showMessage = true)
    ...
    if commander.is_a?(Array) && !commander.empty? && !commander[0].nil?
      pairedBattler = @battle.battlers[commander[0]] rescue nil
      if pairedBattler
  ...
  def pbCheckFormOnWeatherChange(ability_changed = false)
    ...
    if isSpecies?(:CASTFORM)
      if hasActiveAbility?(:FORECAST)
        ...
        pbChangeForm(0, _INTL("¡{1} se ha transformado!", pbThis)) if !RandomizedChallenge.enabled?
      end
    end
    # Cherrim - Flower Gift
