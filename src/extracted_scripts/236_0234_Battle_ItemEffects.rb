...
Battle::ItemEffects::HPHeal.add(:AGUAVBERRY,
  proc { |item, battler, battle, forced|
    next battler.pbConfusionBerry(item, forced, :SPECIAL_DEFENSE,
       _INTL("¡La {2} estaba demasiado amarga para {1}!", battler.pbThis(true), GameData::Item.get(item).name)
    )
  }
)
...
Battle::ItemEffects::HPHeal.add(:FIGYBERRY,
  proc { |item, battler, battle, forced|
    next battler.pbConfusionBerry(item, forced, :ATTACK,
       _INTL("¡La {2} estaba demasiado picante para {1}!", battler.pbThis(true), GameData::Item.get(item).name)
    )
  }
)
...
Battle::ItemEffects::HPHeal.add(:IAPAPABERRY,
  proc { |item, battler, battle, forced|
    next battler.pbConfusionBerry(item, forced, :DEFENSE,
       _INTL("¡La {2} estaba demasiado ácida para {1}!", battler.pbThis(true), GameData::Item.get(item).name)
    )
  }
)
...
Battle::ItemEffects::HPHeal.add(:LANSATBERRY,
  proc { |item, battler, battle, forced|
    ...
    if forced
      ...
      battle.pbDisplay(_INTL("¡{1} ha usado {2} y se ha entusiasmado!", battler.pbThis, itemName))
    end
    next true
  }
)
...
Battle::ItemEffects::HPHeal.add(:MAGOBERRY,
  proc { |item, battler, battle, forced|
    next battler.pbConfusionBerry(item, forced, :SPEED,
       _INTL("¡La {2} estaba demasiado dulce para {1}!", battler.pbThis(true), GameData::Item.get(item).name)
    )
  }
)
...
Battle::ItemEffects::HPHeal.add(:SITRUSBERRY,
  proc { |item, battler, battle, forced|
    ...
    if forced
      ...
      battle.pbDisplay(_INTL("¡{1} ha restaurado su salud con {2}!", battler.pbThis, itemName))
    end
    next true
  }
)
...
Battle::ItemEffects::HPHeal.add(:WIKIBERRY,
  proc { |item, battler, battle, forced|
    next battler.pbConfusionBerry(item, forced, :SPECIAL_ATTACK,
       _INTL("¡La {2} estaba demasiado seca para {1}!", battler.pbThis(true), GameData::Item.get(item).name)
    )
  }
)
...
Battle::ItemEffects::StatusCure.add(:ASPEARBERRY,
  proc { |item, battler, battle, forced|
    ...
    battle.pbDisplay(_INTL("¡{1} se ha descongelado con {2}!", battler.pbThis, itemName)) if !forced
    next true
  }
)
...
Battle::ItemEffects::StatusCure.add(:MENTALHERB,
  proc { |item, battler, battle, forced|
    ...
    if battler.effects[PBEffects::Attract] >= 0
      if forced
        battle.pbDisplay(_INTL("¡{1} se ha liberado del enamoramiento!", battler.pbThis))
      else
        battle.pbDisplay(_INTL("¡{1} se ha liberado del enamoramiento con {2}!",
           battler.pbThis, itemName))
      end
      battler.pbCureAttract
    end
    battle.pbDisplay(_INTL("¡{1} ya se ha olvidado de la mofa!", battler.pbThis)) if battler.effects[PBEffects::Taunt] > 0
    battler.effects[PBEffects::Taunt] = 0
...
Battle::ItemEffects::StatusCure.add(:PERSIMBERRY,
  proc { |item, battler, battle, forced|
    ...
    battler.pbCureConfusion
    if forced
      battle.pbDisplay(_INTL("¡{1} ya no está confuso!", battler.pbThis))
    else
      battle.pbDisplay(_INTL("¡{1} ya no está confuso gracias a {2}!", battler.pbThis,
         itemName))
    end
    next true
  }
...
Battle::ItemEffects::StatusCure.add(:RAWSTBERRY,
  proc { |item, battler, battle, forced|
    ...
    battle.pbCommonAnimation("EatBerry", battler) if !forced
    battler.pbCureStatus(forced)
    battle.pbDisplay(_INTL("¡{1} se ha curado de las quemaduras con {2}!", battler.pbThis, itemName)) if !forced
    next true
  }
)
...
Battle::ItemEffects::PriorityBracketUse.add(:CUSTAPBERRY,
  proc { |item, battler, battle|
    ...
    battle.pbDisplay(_INTL("¡{1} se mueve primero gracias a {2}!", battler.pbThis, battler.itemName))
...
Battle::ItemEffects::OnMissingTarget.add(:BLUNDERPOLICY,
  proc { |item, user, target, move, hit_num, battle|
    ...
    battle.pbDisplay(_INTL("{1} ha desaparecido al haber cumplido su función.", user.itemName))
...
Battle::ItemEffects::DamageCalcFromUser.add(:FISTPLATE,
  proc { |item, user, target, move, mults, power, type|
...
Battle::ItemEffects::DamageCalcFromUser.add(:DREADPLATE,
  proc { |item, user, target, move, mults, power, type|
    mults[:power_multiplier] *= 1.25 if type == :DARK
  }
)
...
Battle::ItemEffects::DamageCalcFromUser.add(:FLAMEPLATE,
  proc { |item, user, target, move, mults, power, type|
...
Battle::ItemEffects::DamageCalcFromUser.add(:DRACOPLATE,
  proc { |item, user, target, move, mults, power, type|
...
Battle::ItemEffects::DamageCalcFromUser.copy(:HARDSTONE, :ROCKINCENSE)

Battle::ItemEffects::DamageCalcFromUser.add(:STONEPLATE,
  proc { |item, user, target, move, mults, power, type|
...
Battle::ItemEffects::DamageCalcFromUser.add(:LIGHTBALL,
  proc { |item, user, target, move, mults, power, type|
    ...
    mults[:attack_multiplier] *= 1.5 if user.isSpecies?(:RAICHU)
  }
)
...
Battle::ItemEffects::DamageCalcFromUser.add(:ZAPPLATE,
  proc { |item, user, target, move, mults, power, type|
    mults[:power_multiplier] *= 1.25 if type == :ELECTRIC
  }
)
...
Battle::ItemEffects::DamageCalcFromUser.add(:IRONPLATE,
  proc { |item, user, target, move, mults, power, type|
    mults[:power_multiplier] *= 1.25 if type == :STEEL
  }
)
...
Battle::ItemEffects::DamageCalcFromUser.copy(:MIRACLESEED, :ROSEINCENSE)

Battle::ItemEffects::DamageCalcFromUser.add(:MEADOWPLATE,
  proc { |item, user, target, move, mults, power, type|
...
Battle::ItemEffects::DamageCalcFromUser.copy(:MYSTICWATER, :SEAINCENSE, :WAVEINCENSE)

Battle::ItemEffects::DamageCalcFromUser.add(:SPLASHPLATE,
  proc { |item, user, target, move, mults, power, type|
    mults[:power_multiplier] *= 1.25 if type == :WATER
  }
)
...
Battle::ItemEffects::DamageCalcFromUser.add(:ICICLEPLATE,
  proc { |item, user, target, move, mults, power, type|
...
Battle::ItemEffects::DamageCalcFromUser.add(:PIXIEPLATE,
  proc { |item, user, target, move, mults, power, type|
    mults[:power_multiplier] *= 1.25 if type == :FAIRY
  }
)
...
Battle::ItemEffects::DamageCalcFromUser.add(:TOXICPLATE,
  proc { |item, user, target, move, mults, power, type|
    mults[:power_multiplier] *= 1.25 if type == :POISON
  }
)
...
Battle::ItemEffects::DamageCalcFromUser.add(:SKYPLATE,
  proc { |item, user, target, move, mults, power, type|
    mults[:power_multiplier] *= 1.25 if type == :FLYING
  }
)
...
Battle::ItemEffects::DamageCalcFromUser.add(:BLANKPLATE,
  proc { |item, user, target, move, mults, power, type|
    mults[:power_multiplier] *= 1.25 if type == :NORMAL
  }
)
...
Battle::ItemEffects::DamageCalcFromUser.add(:INSECTPLATE,
  proc { |item, user, target, move, mults, power, type|
    mults[:power_multiplier] *= 1.25 if type == :BUG
  }
)
...
Battle::ItemEffects::DamageCalcFromUser.add(:EARTHPLATE,
  proc { |item, user, target, move, mults, power, type|
    mults[:power_multiplier] *= 1.25 if type == :GROUND
  }
)
...
Battle::ItemEffects::DamageCalcFromUser.add(:SPOOKYPLATE,
  proc { |item, user, target, move, mults, power, type|
    mults[:power_multiplier] *= 1.25 if type == :GHOST
  }
)
...
Battle::ItemEffects::DamageCalcFromUser.copy(:TWISTEDSPOON, :ODDINCENSE)

Battle::ItemEffects::DamageCalcFromUser.add(:MINDPLATE,
  proc { |item, user, target, move, mults, power, type|
    mults[:power_multiplier] *= 1.25 if type == :PSYCHIC
  }
)
...
Battle::ItemEffects::DamageCalcFromTarget.add(:SUPEREVIOLITE,
  proc { |item, user, target, move, mults, power, type|
    # NOTE: Super Eviolite si el Pokemon puede evolucionar 2 veces
    #       Gana un 50% de estadisticas de ataque, defensa
    if target.pokemon.species_data.get_evolutions(true).length > 1
      mults[:defense_multiplier] *= 1.5
    end
...
Battle::ItemEffects::DamageCalcFromUser.add(:SUPEREVIOLITE,
  proc { |item, user, target, move, mults, power, type|
    # NOTE: Super Eviolite si el Pokemon puede evolucionar 2 veces
    #       Gana un 50% de estadisticas de ataque, defensa
    if user.pokemon.species_data.get_evolutions(true).length > 1
      mults[:attack_multiplier]  *= 1.5
    end
  }
)

Battle::ItemEffects::SpeedCalc.add(:SUPEREVIOLITE,
  proc { |item, battler, mult|
    # NOTE: Super Eviolite si el Pokemon puede evolucionar 2 veces
    #       Gana un 50% de velocidad
    if battler.pokemon.species_data.get_evolutions(true).length > 1
      next mult * 1.5
    end
  }
)
...
Battle::ItemEffects::OnBeingHit.add(:WEAKNESSPOLICY,
  proc { |item, user, target, move, battle|
    ...
    battle.pbDisplay(_INTL("{1} ha desaparecido al haber cumplido su función.", target.itemName))
    target.pbHeldItemTriggered(item)
  }
)
...
Battle::ItemEffects::AfterMoveUseFromUser.add(:LIFEORB,
  proc { |item, user, targets, move, numHits, battle|
    ...
    battle.pbDisplay(_INTL("¡{1} ha perdido algunos PS!", user.pbThis))
...
Battle::ItemEffects::OnEndOfUsingMove.add(:LEPPABERRY,
  proc { |item, battler, battle, forced|
    ...
    if forced
      battle.pbDisplay(_INTL("¡{1} ha recuperado los PP de {2}!", battler.pbThis, moveName))
    else
      battle.pbDisplay(_INTL("¡{1} ha recuperado los PP de {3} gracias a {2}!", battler.pbThis, itemName, moveName))
    end
    next true
  }
)
  ...
  proc { |item, battler, battle|
    if battler.pbHasType?(:POISON)
      ...
      battler.pbTakeEffectDamage(battler.totalhp / 8) do |hp_lost|
        battle.pbDisplay(_INTL("¡{1} ha resultado dañado por {2}!", battler.pbThis, battler.itemName))
      end
    end
  }
)
...
Battle::ItemEffects::EndOfRoundEffect.add(:TOXICORB,
  proc { |item, battler, battle|
    ...
    battler.pbPoison(nil, _INTL("¡{1} ha sido gravemente envenenado por {2}!",
...
Battle::ItemEffects::OnSwitchIn.add(:AIRBALLOON,
  proc { |item, battler, battle|
    battle.pbDisplay(_INTL("¡{1} está flotando con {2}!",
       battler.pbThis, battler.itemName))
  }
)
