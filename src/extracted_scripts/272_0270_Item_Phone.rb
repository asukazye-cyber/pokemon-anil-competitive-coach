...
class Phone
  ...
  def self.add(*args)
    ...
    if ret
      ...
      pbMessage(_INTL("\\me[Register phone]¡Has registrado {1} en el Pokégear!", contact.display_name) + "\\wtnp[60]")
    end
    return ret
  end
...
class Phone
  module Call
    ...
    def can_call_contact?(contact)
      return false if !contact
      if !can_make?
        pbMessage(_INTL("El teléfono no tiene señal aquí..."))
        return false
      end
      return true if !contact.trainer?
      if contact.map_id == $game_map.map_id
        pbMessage(_INTL("El entrenador está muy cerca.\n¡Habla con él en persona!"))
        return false
      end
      ...
      if !caller_map_metadata || !caller_map_metadata.town_map_position ||
        ...
        pbMessage(_INTL("El entrenador está fuera de rango."))
        return false
      end
      return true
    end
    ...
    def make_incoming
      ...
      if contact.common_event_call?
        if !pbCommonEvent(contact.common_event_id)
          pbMessage(_INTL("No se han definido los mensajes de {1}.\nNo se puede llamar al evento común {2}.",
    ...
    def make_outgoing(*args)
      ...
      raise _INTL("No se encuentra el contacto del teléfono dado: {1}.", args.inspect) if !contact
