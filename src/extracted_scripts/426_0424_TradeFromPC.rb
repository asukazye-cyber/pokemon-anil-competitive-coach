...
def pbChoosePokemonForTradePC(wanted)
  ...
  @chosen = pbChooseTradablePokemonPC(variableNumber, nameVarNumber, wanted)
end

def pbChooseTradablePokemonPC(variableNumber, nameVarNumber, wanted, ableProc = proc{ |poke| !poke.egg? && !poke.shadowPokemon? && !poke.fainted? }, allowIneligible = false)
  chosen = -1
  variableNumber = 1
  nameVarNumber = 2
  pbFadeOutIn {
    ...
    chosen = screen.pbChoosePokemonFromPC(wanted, ableProc)
  }
...
def pbStartTradePC(newpoke, trainerGender = 0, level = 0, nickname = nil, trainerName = nil)
  ...
  if newpoke.is_a?(Pokemon)
    ...
    RandomizedChallenge.resume_random_species if RandomizedChallenge.enabled? && RandomizedChallenge.wild_paused?
    yourPokemon = Pokemon.new(species_data.id, (level > 0 ? level : myPokemon.level))
...
class PokemonStorageScreen
  def pbChoosePokemonFromPC(wanted = nil, ableProc = nil)
    ...
    loop do
      ...
      if selected.nil?
        ...
        when 0   # Select
          ...
          if !wanted || pokemon.species == wanted
            retval = selected
            break
          else
