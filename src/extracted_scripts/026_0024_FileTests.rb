...
# ⚠️ A RESPOSTA NAO MUDA, MAS A PERGUNTA CUSTA — SOBRETUDO NO ANDROID.
#
# Cada chamada sonda o disco duas vezes (.png e .gif) por cada raiz do
# RTP.eachPathFor, e sonda na mesma quando o ficheiro NAO existe. Isso acontece
# imenso: o Sprite_Character#refresh_graphic pergunta sempre por
# "Graphics/Skins/<nome>" antes de cair em "Graphics/Characters/<nome>", e para
# um NPC essa pergunta falha sempre — mas paga as sondagens todas.
#
# Medido no JoiPlay: entrar num mapa custava 6,1 s so em carregamento de
# imagens, com 91 pedidos para 21 ficheiros distintos.
#
# A resposta e estavel durante a sessao. A unica excepcao sao as skins, que
# chegam pela rede em jogo — e essas limpam esta tabela ao gravar (ver o
# 002_Script_Skins). Guardam-se tambem os FALHADOS, que sao o caso caro.
$anil_resolve_cache = {} if !defined?($anil_resolve_cache) || $anil_resolve_cache.nil?

def pbResolveBitmap(x)
  return nil if !x
  $anil_resolve_cache = {} if $anil_resolve_cache.nil?
  chave = x.to_s
  return $anil_resolve_cache[chave] if $anil_resolve_cache.key?(chave)
  $anil_resolve_cache[chave] = anil_resolve_bitmap_sem_cache(x)
end

def anil_resolve_bitmap_sem_cache(x)
  return nil if !x
