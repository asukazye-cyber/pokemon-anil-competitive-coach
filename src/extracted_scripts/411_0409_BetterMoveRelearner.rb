...
class UI::MoveReminderVisuals < UI::BaseVisuals
  ...
  BUTTON_DOWN_X      = 132
  BUTTON_DOWN_Y      = Graphics.height - 100
  ...
  def moves=(move_list)
    ...
    @cursor&.visible = false if @moves.empty? && @cursor
    refresh
  end
  ...
  def draw_buttons
    ...
    draw_image(@bitmaps[:buttons], BUTTON_DOWN_X, BUTTON_DOWN_Y, BUTTON_UP_WIDTH, 0, BUTTON_DOWN_WIDTH, BUTTON_DOWN_HEIGHT) if @top_index > 0
  end
...
def pbGetTMMoves(pokemon)
  ...
  for item_aux in $bag.pockets[4]
    ...
    if item.is_machine?
      machine = item.move
      move = GameData::Move.get(machine)
      tmorhm = item.is_HM? ? "MO" : "MT"
      if pokemon.compatible_with_move?(move) && !pokemon.hasMove?(move)
