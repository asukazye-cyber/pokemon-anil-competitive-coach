...
def pbCanUsePokeRadar?
  ...
  if $PokemonGlobal.bicycle
    pbMessage(_INTL("No se puede usar mientras vas en bicicleta."))
    return false
  end
  # Debug
  ...
  if $PokemonGlobal.pokeradarBattery && $PokemonGlobal.pokeradarBattery > 0
    pbMessage(_INTL("¡La batería se ha agotado!\nPara recargarla, tienes que andar {1} pasos.",
...
def pbPokeRadarHighlightGrass(showmessage = true)
  ...
  4.times do |i|
    ...
    if $game_temp.poke_radar_data && $game_temp.poke_radar_data[2] > 0
      chain = [$game_temp.poke_radar_data[2], 40].min
      has_charm = ($bag.has?(:SHINYCHARM) || $bag.has?(:KEYSHINYCHARM) rescue false)
      target_denom = has_charm ? 250 : 500
      base_denom = 65_536 / Settings::SHINY_POKEMON_CHANCE
      denom = base_denom - ((base_denom - target_denom) * (chain / 40.0))
      denom = [denom, target_denom].max
      v = (65_536 / denom.to_f).ceil
  ...
  if grasses.length == 0
    ...
    pbMessage(_INTL("El parterre de hierba no se ha movido...")) if showmessage
    pbPokeRadarCancel
  else
