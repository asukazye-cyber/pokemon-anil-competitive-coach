...
class BattlePalaceBattle < Battle
  ...
  def pbPinchChange(battler)
    ...
    when :QUIET, :BASHFUL, :NAIVE, :QUIRKY, :HARDY, :DOCILE, :SERIOUS
      pbDisplay(_INTL("¡{1} está ansioso de más!", battler.pbThis))
    when :CAREFUL, :RASH, :LAX, :SASSY, :MILD, :TIMID
      pbDisplay(_INTL("¡{1} empezó a gruñir fuertemente!", battler.pbThis))
    when :GENTLE, :ADAMANT, :HASTY, :LONELY, :RELAXED, :NAUGHTY
      pbDisplay(_INTL("¡Un brillo aparece en los ojos de {1}!", battler.pbThis(true)))
    when :JOLLY, :BOLD, :BRAVE, :CALM, :IMPISH, :MODEST
      pbDisplay(_INTL("¡{1} se está posicionando!", battler.pbThis))
    end
  end
