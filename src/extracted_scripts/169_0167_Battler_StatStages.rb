class Battle::Battler
  ...
  def pbLowerAttackStatStageIntimidate(user)
    ...
    if @effects[PBEffects::Substitute] > 0
      if Battle::Scene::USE_ABILITY_SPLASH
        @battle.pbDisplay(_INTL("¡El sustituto recibe el efecto en lugar de {1}!", pbThis(true)))
      else
  ...
  def pbLowerSpecialAttackStatStageIntimidate(user)
    ...
    # Paldea - Gen 9
    if !hasActiveAbility?(:CONTRARY) && @effects[PBEffects::Substitute] == 0
      if itemActive? && Battle::ItemEffects.triggerStatLossImmunity(self.item, self, :SPECIAL_ATTACK, @battle, true)
        return false
      end
    end
    # NOTE: Substitute intentionally blocks Intimidate even if self has Contrary.
    ...
    if Settings::MECHANICS_GENERATION >= 8 && hasActiveAbility?([:OBLIVIOUS, :OWNTEMPO, :INNERFOCUS, :SCRAPPY])
      @battle.pbShowAbilitySplash(self)
      if Battle::Scene::USE_ABILITY_SPLASH
        @battle.pbDisplay(_INTL("¡{2} de {1} no puede bajar más!", pbThis(true), GameData::Stat.get(:SPECIAL_ATTACK).name))
      else
        @battle.pbDisplay(_INTL("¡{2} de {1} evitó que bajara su {3}!", pbThis(true), abilityName,
