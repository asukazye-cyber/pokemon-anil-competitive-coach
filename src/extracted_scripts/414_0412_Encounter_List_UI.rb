...
USER_DEFINED_NAMES = {
    ...
    :PokeRadar => "PokéRadar",
    :LandClassic => "Hierba",
    :LandClassicDay => "Hierba (día)",
    :LandClassicNight => "Hierba (noche)",
    :LandClassicMorning => "Hierba (mañana)",
    :LandClassicAfternoon => "Hierba (medio día)", 
    :LandClassicEvening => "Hierba (atardecer)",
    :CaveClassic => "Cueva",
    :CaveClassicDay => "Cueva (día)",
    :CaveClassicNight => "Cueva (noche)",
    :CaveClassicMorning => "Cueva (mañana)",
    :CaveClassicAfternoon => "Cueva (medio día)",
    :CaveClassicEvening => "Cueva (atardecer)",
    :WaterClassic => "Surfeando",
    :WaterClassicDay => "Surfeando (día)",
    :WaterClassicNight => "Surfeando (noche)",
    :WaterClassicMorning => "Surfeando (mañana)",
    :WaterClassicAfternoon => "Surfeando (medio día)",
    :WaterClassicEvening => "Surfeando (atardecer)",
    :OldRodClassic => "Pescando",
    :GoodRodClassic => "Pescando",
    :SuperRodClassic => "Pescando",
    :RockSmashClassic => "Golpe Roca",
    :HeadbuttClassicLow => "Cabezazo (Raro)",
    :HeadbuttClassicHigh => "Cabezazo (Común)",
    :BugContestClassic => "Concurso de Bichos",
    :PokeRadarClassic => "PokéRadar"
}
...
class EncounterList_Scene
  ...
  def initialize
    ...
    if @encounter_data
      # @encounter_tables = Marshal.load(Marshal.dump(@encounter_data.types)) # Forma antigua sin filtrar modo clásico.
      all_tables = Marshal.load(Marshal.dump(@encounter_data.types))
      if $game_switches[MODO_CLASICO]
        # Filtra solo los encuentros que terminan en Classic
        @encounter_tables = all_tables.select { |k, _| k.to_s.end_with?("Classic") }
      else
        # No mostrar ningún tipo de encuentro si el switch está apagado
        @encounter_tables = all_tables.reject { |k, _| k.to_s.end_with?("Classic") }
      end
  ...
  def drawPresent
    ...
    enc_array.each do |s|
      ...
      @sprites["icon_#{i}"].pbSetParams(species_data.species, 0, species_data.form, false)
    ...
    loctext += sprintf("<ac><c2=7FFF5EF7>Encuentros totales de la zona: %s</c2></ac>",enc_array.length)
