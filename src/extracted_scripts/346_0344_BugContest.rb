...
class BugContestState
  ...
  CONTESTANT_NAMES = [
    _INTL("Cazabichos Edu"),
    _INTL("Cazabichos Benito"),
    _INTL("Cazabichos José"),
    _INTL("Campista Barry"),
    _INTL("Entre. Guay Nico"),
    _INTL("Chica Alba"),
    _INTL("Dominguero Cintia"),
    _INTL("Joven Samuel")
  ]
...
EventHandlers.add(:on_frame_update, :bug_contest_counter,
  proc {
    ...
    pbMessage(_INTL("ALTAVOZ: ¡BEEEEEP!"))
    pbMessage(_INTL("¡Se acabó el tiempo!"))
    pbBugContestState.pbStartJudging
  }
)
...
MenuHandlers.add(:pause_menu, :quit_bug_contest, {
  ...
  "effect"    => proc { |menu|
    menu.pbHideMenu
    if pbConfirmMessage(_INTL("¿Quieres terminar ahora el Concurso de Captura de Bichos?"))
      menu.pbEndScene
      pbBugContestState.pbStartJudging
      next true
    end
