...
class PokemonPokedexInfo_Scene
  ...
  def pbFindEncounter(enc_types, species)
    ...
    # Filter encounter types based on MODO_CLASICO switch
    filtered_types = {}
    enc_types.each do |enc_type, slots|
      type_name = enc_type.to_s
      if $game_switches[MODO_CLASICO]
        # Only include Classic encounter types
        filtered_types[enc_type] = slots if type_name.end_with?("Classic")
      else
        # Only include non-Classic encounter types
        filtered_types[enc_type] = slots if !type_name.end_with?("Classic")
      end
    end
    
    filtered_types.each_value do |slots|
      next if !slots
  ...
  def pbGetEncounterPoints
    ...
    GameData::Encounter.each_of_version($PokemonGlobal.encounter_version) do |enc_data|
      ...
      # ⚠️ HA MAPAS COM ENCONTROS QUE NAO ESTAO NO MAPA DA REGIAO.
      #
      # `town_map_position` e nil quando o mapa nao tem `MapPosition` no
      # map_metadata — e isso e legitimo: uma area propositadamente fora do
      # mapa da regiao nao tem onde ser marcada. Sao os 305-308 (Safari
      # Abandonado) que o trouxeram a luz.
      #
      # Sem esta guarda, abrir a ficha de QUALQUER especie que apareca so num
      # desses mapas dava `undefined method [] for nil:NilClass` e fechava o
      # jogo. E nao e um caso raro que se corrige uma vez: qualquer area nova
      # sem posicao no mapa volta a parti-la.
      #
      # Repare-se que o proprio Essentials ja sabe disto — a linha 20 deste
      # ficheiro le `$game_map.metadata&.town_map_position` e trata o nil. Aqui
      # esqueceram-se, so isso.
      next if !mappos || mappos[0] != @region   # Sem posicao, ou fora da regiao
