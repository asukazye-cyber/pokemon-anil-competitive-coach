    ...
    using(cmdwindow = Window_CommandPokemonColor.new(commands)) do
      ...
      loop do
        ...
        if Input.trigger?(Input::BACK)
          ...
          if mousepos
            viewport = cmdwindow.viewport rescue nil
            v_x = (viewport && !viewport.disposed?) ? (viewport.rect.x - viewport.ox) : 0
            v_y = (viewport && !viewport.disposed?) ? (viewport.rect.y - viewport.oy) : 0
            border_x = cmdwindow.respond_to?(:borderX) ? cmdwindow.borderX : 32
            border_y = cmdwindow.respond_to?(:borderY) ? cmdwindow.borderY : 32
            cx = mousepos[0] - (cmdwindow.x + v_x + border_x / 2)
            cy = mousepos[1] - (cmdwindow.y + v_y + border_y / 2)
            clicked_item = -1
            cmdwindow.itemCount.times do |i|
              rect = cmdwindow.itemRect(i)
...
class PokemonPartyScreen
  ...
  def pbPokemonScreen
    ...
    if ($player.has_box_link || $bag.has?(:POKEMONBOXLINK) || $bag.has?(:KEYPOKEMONBOXLINK)) &&
