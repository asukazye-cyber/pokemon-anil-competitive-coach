...
class PokemonRuleSet
  ...
  def maxLength
    return (@number < 0) ? max_party_size : @number
  end
  ...
  def maxTeamLength
    return [max_party_size, self.maxLength].max
  end
  ...
  def isValid?(team, error = nil)
    if team.length < self.minLength
      error.push(_INTL("Elige un Pokémon.")) if error && self.minLength == 1
      error.push(_INTL("Se necesitan {1} Pokémon.", self.minLength)) if error && self.minLength > 1
      return false
    elsif team.length > self.maxLength
      error&.push(_INTL("No pueden entrar más de {1} Pokémon.", self.maxLength))
      return false
    end
    team.each do |pkmn|
      next if isPokemonValid?(pkmn)
      if pkmn
        error&.push(_INTL("{1} no está permitido.", pkmn.name))
      elsif error
        error.push(_INTL("Este equipo no está permitido."))
      end
      return false
    end
    @teamRules.each do |rule|
