...
class Battle::Move::OHKO < Battle::Move::FixedDamageMove
  ...
  def pbHitEffectivenessMessages(user, target, numTargets = 1)
    super
    @battle.pbDisplay(_INTL("¡K.O. en un golpe!")) if target.fainted?
  end
end
...
class Battle::Move::EnsureNextCriticalHit < Battle::Move
  ...
  def pbEffectGeneral(user)
    ...
    @battle.pbDisplay(_INTL("¡{1} se prepara para que el siguiente golpe sea crítico!", user.pbThis))
  end
end
...
class Battle::Move::StartPreventCriticalHitsAgainstUserSide < Battle::Move
  ...
  def pbEffectGeneral(user)
    ...
    @battle.pbDisplay(_INTL("¡Conjuro protege a {1} de golpes críticos!", user.pbTeam(true)))
  end
end
...
class Battle::Move::UserEnduresFaintingThisTurn < Battle::Move::ProtectMove
  ...
  def pbProtectMessage(user)
    @battle.pbDisplay(_INTL("¡{1} se prepara para resistir los ataques!", user.pbThis))
  end
end
...
class Battle::Move::ProtectUserSideFromDamagingMovesIfUserFirstTurn < Battle::Move
  ...
  def pbEffectGeneral(user)
    ...
    @battle.pbDisplay(_INTL("¡{1} va a usar un tatami para bloquear ataques!", user.pbThis))
  end
end
...
class Battle::Move::ProtectUserSideFromStatusMoves < Battle::Move
  ...
  def pbEffectGeneral(user)
    ...
    @battle.pbDisplay(_INTL("¡Truco defensa protegió a {1}!", user.pbTeam(true)))
  end
end
...
class Battle::Move::HoopaRemoveProtectionsBypassSubstituteLowerUserDef1 < Battle::Move::StatDownMove
  ...
  #     @battle.pbDisplay(_INTL("Sin embargo, {1} no ha podido ponerlo en práctica!", user.pbThis(true)))
  #     return true
  #   elsif user.form != 1
  #     @battle.pbDisplay(_INTL("¡Pero {1} no ha podido ponerlo en práctica en su forma actual!", user.pbThis(true)))
  #     return true
  #   end
  #   return false
  # end
...
class Battle::Move::EnsureNextMoveAlwaysHits < Battle::Move
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("{1} apuntó a {2}!", user.pbThis, target.pbThis(true)))
  end
end
...
class Battle::Move::StartNegateTargetEvasionStatStageAndGhostImmunity < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("¡{1} fue identificado!", target.pbThis))
  end
end
  ...
  def pbOnStartUse(user, targets)
    if user.hasLegendPlateJudgment? && !targets.empty?
      ...
      if user.form != newForm
        ...
        _INTL("{1} se transformó al tipo {2}!", user.pbThis, typeName))
      end
    end
  end
...
class Battle::Move::TypeDependsOnUserMorpekoFormRaiseUserSpeed1 < Battle::Move::RaiseUserSpeed1
  ...
  def pbBaseType(user)
    return :DARK if user.isSpecies?(:MORPEKO) && user.form == 1 && GameData::Type.exists?(:DARK)
    return @type
  end
end
...
class Battle::Move::FlinchTargetFailsIfTargetNotUsingPriorityMove < Battle::Move::FlinchTarget
  def pbMoveFailed?(user, targets)
    ...
    targets.each do |b|
      ...
      next if !choices[2].is_a?(Battle::Move)
      next if !choices[2] || !choices[2].is_a?(Battle::Move) || !choices[2]&.damagingMove?
