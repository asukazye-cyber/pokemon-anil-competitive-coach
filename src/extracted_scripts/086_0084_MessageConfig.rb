...
module MessageConfig
  ...
  FONT_SIZE                = 22
  FONT_Y_OFFSET            = 8
  SMALL_FONT_NAME          = "Anil Sans"
  SMALL_FONT_SIZE          = 17
  SMALL_FONT_Y_OFFSET      = 8
  TINY_FONT_SIZE           = 12
  TINY_FONT_Y_OFFSET       = 7
  NARROW_FONT_NAME         = "Anil Sans"
  NARROW_FONT_SIZE         = 22
  ...
  # Indice de windowskin sempre valido.
  #
  # ⚠️ O `if $PokemonSystem` original confirmava que o OBJETO existe, mas nao que
  # a opcao dentro dele e um numero. Num save cujo `frame`/`textskin` veio nil
  # (save antigo, ou vindo de outro build, onde a opcao ainda nao existia), o
  # `Settings::MENU_WINDOWSKINS[nil]` levantava
  # "TypeError: no implicit conversion from nil to integer".
  #
  # E o crash era grave por ONDE acontecia: qualquer janela de texto passa por
  # aqui. Na recuperacao de partida, o `rescue` de recuperar_partida chama
  # pbMessage para mostrar o erro — e era o proprio pbMessage que rebentava,
  # escondendo a falha original e derrubando o jogo na tela de load (2026-08-02).
  #
  # Repor a opcao no $PokemonSystem (e nao so devolver 0) faz o save curar-se
  # sozinho na proxima gravacao, senao o menu de Opcoes tropeca no mesmo nil.
  def self.indice_windowskin_valido(atributo, lista)
    idx = ($PokemonSystem && $PokemonSystem.respond_to?(atributo)) ? $PokemonSystem.send(atributo) : nil
    return idx if idx.is_a?(Integer) && idx >= 0 && idx < lista.length
    if $PokemonSystem && $PokemonSystem.respond_to?("#{atributo}=")
      $PokemonSystem.send("#{atributo}=", 0) rescue nil
    end
    0
  end

  def self.pbDefaultSystemFrame
    idx = indice_windowskin_valido(:frame, Settings::MENU_WINDOWSKINS)
    path = File.join("Graphics", "Windowskins", Settings::MENU_WINDOWSKINS[idx])
    return pbResolveBitmap(path) || ""
  end

  def self.pbDefaultSpeechFrame
    idx = indice_windowskin_valido(:textskin, Settings::SPEECH_WINDOWSKINS)
    path = File.join("Graphics", "Windowskins", Settings::SPEECH_WINDOWSKINS[idx])
    return pbResolveBitmap(path) || ""
  end
...
if !defined?(_INTL)
  def _INTL(*args)
  args.each_with_index { |v, i| args[i] = PTBR_TEXT.t(v) if v.is_a?(String) }
    string = args[0].clone
