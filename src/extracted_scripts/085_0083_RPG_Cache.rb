  ...
  module Cache
    ...
    def self.tileEx(filename, tile_id, hue, width = 1, height = 1)
      ...
      if ret
        ...
        @cache[key] = ret
        # ⚠️ A CACHE TEM DE FICAR COM UMA REFERENCIA SUA. SEM ISTO NAO E CACHE.
        #
        # O BitmapWrapper nasce com refcount 1, e essa unica referencia era
        # entregue a quem chamou. O Sprite_Character faz `@charbitmap.dispose`
        # no refresco seguinte, o contador cai a zero, e o bitmap e destruido —
        # com a entrada a ficar na @cache ja morta. O `fromCache` devolve nil
        # para bitmaps libertados, portanto a proxima vez volta a passar por
        # aqui e recomeca tudo.
        #
        # E "tudo" e caro de uma forma desproporcionada: para recortar UM tile
        # de 32x32, o bloco acima le o tileset INTEIRO do disco (o Exterior2 sao
        # 256x23370, 22,8 MB descodificados) e a seguir deita-o fora com o
        # `tileset.dispose` — que ainda por cima mata a entrada do tileset na
        # propria @cache, porque e o mesmo BitmapWrapper.
        #
        # Resultado medido: um so evento de tile custava 43-46 ms por entrada de
        # mapa, mais de 80% de todo o tempo de sprites, em TODAS as entradas.
        # Com o addRef, o recorte sobrevive ao dispose do sprite e o tileset e
        # lido uma vez por tile em toda a sessao. Sao 4 KB por tile guardados.
        ret.addRef
      end
      return ret
    end
