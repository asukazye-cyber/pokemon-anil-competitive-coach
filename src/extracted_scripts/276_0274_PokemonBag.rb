...
class PokemonBag
  ...
  def remove_items_by_flag(flag = "Healing", store_deleted = nil)
    deleted = {}
    @pockets.each do |pocket|
      pocket.each do |item|
        next unless GameData::Item.get(item[0]).flags.include?(flag)
        deleted[item[0]] = GameData::Item.get(item[0]).flags.include?("Berry") ? item[1] - 2 : item[1]
      end
    end
    pbSet(store_deleted, deleted) if store_deleted
    deleted.each { |item, qty| remove(item, qty) }
      end

  def restore_items(variable)
    deleted = pbGet(variable)
    return if !deleted || !deleted.is_a?(Hash) || deleted.empty?
    deleted.each { |item, qty| add(item, qty) }
    pbSet(variable, nil)
  end
  ...
  def has_fossil?(exclude: [])
    @pockets.each do |pocket|
      pocket.each do |item|
        next unless GameData::Item.get(item[0]).is_fossil? && !exclude.include?(item[0])
        return true
      end
    end
    return false
...
module ItemStorageHelper
  ...
  def self.remove(items, item, qty)
    ...
    items.compact!
    $bag.unmark_as_new(item) if $bag && ret 
    return ret
  end
end
