...
class PokemonLoad_Scene
  ...
  def pbUpdate
    ...
    if oldi != newi
      ...
      while @sprites["panel#{newi}"].y > Graphics.height - 80
        ...
        @sprites["leftarrow"].y -= 48
        @sprites["rightarrow"].y -= 48
  ...
  def pbSetParty(trainer)
    ...
    if meta
      ...
      # Fallback: se não tiver skin no save, tenta ler do arquivo de config
      if !filename
        cfg_file = "multiplayer_player.txt"
        if File.exist?(cfg_file)
          begin
            File.readlines(cfg_file).each do |line|
              if line.include?("char=")
                filename = pbResolvePlayerCharsetName(line.split("=")[1].to_s.strip)
                break
              end
            end
          rescue
      ...
      if !@sprites["player"].bitmap
        # Se falhou (pode ser skin inexistente no fallback), tenta o padrão absoluto
        filename = pbGetPlayerCharset(meta.walk_charset, trainer, true)
        @sprites["player"] = TrainerWalkingCharSprite.new(filename, @viewport)
      end
      if !@sprites["player"].bitmap
...
class PokemonLoadScreen
  ...
  def pbStartLoadScreen
    # Updater de terceiro removido em 2026-07-19 (apontava para pastebin externo).
    commands = []
    cmd_continue     = -1
    cmd_multiplayer  = -1
    cmd_new_game     = -1
    ...
    loop do
      ...
      when cmd_multiplayer
        ...
        if defined?(AnilLanRework)
          AnilLanRework.multiplayer_mode = true
          # ⚠️ ESTE METODO E CODIGO MORTO.
          #
          # O plugin "013 Multi Save" REDEFINE o pbStartLoadScreen inteiro e
          # monta a propria lista de comandos — nada aqui corre no jogo real.
          # A protecao contra entrar com um save desatualizado vive no
          # download_and_overwrite_save_file (MOD 101), que e o unico ponto que
          # os DOIS caminhos partilham e que nao esta dentro do binario do
          # plugin.
        ...
        # Agenda a conexão automática assim que o mapa iniciar
        if defined?(AnilLanRework)
      ...
      when cmd_recover
        ...
        if defined?(Multiplayer_Whitelist)
          Multiplayer_Whitelist.recuperar_partida
        end
        return
      when cmd_new_game
        @scene.pbEndScene
