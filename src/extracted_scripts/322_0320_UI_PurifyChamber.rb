...
class PurifyChamber
  ...
  def update
    NUMSETS.times do |set|
      ...
      pbMessage(_INTL("¡Tu {1} en la Cámara de Purificación está listo para ser purificado!",
...
#    end
  ...
  def pbPlace(pkmn, position)
    ...
    if pkmn.egg?
      @scene.pbDisplay(_INTL("No se pueden depositar Huevos aquí."))
      return false
    end
    if position == 0
      if pkmn.shadowPokemon?
        ...
        @scene.pbDisplay(_INTL("Solo puedes dejar Pokémon Oscuros."))
        return false
      end
    elsif position >= 1
      if pkmn.shadowPokemon?
        @scene.pbDisplay(_INTL("No se puede dejar un Pokémon Oscuro ahí."))
        return false
      else
        oldpkmn = PurifyChamberHelper.pbGetPokemon(@chamber, position)
        if oldpkmn
  ...
  def pbOnPlace(pkmn)
    ...
    if @chamber.setCount(set) == 0 && @chamber.isPurifiableIgnoreRegular?(set)
      ...
      @scene.pbDisplay(
        _INTL("Este {1} está listo para abrir su corazón. Sin embargo, debe haber al menos un Pokémon normal en el set para llevar a cabo una ceremonia de purificación.",
              pkmn.name)
      )
    end
  end
  ...
  def pbOpenSetDetail
    ...
    loop do
      ...
      when 0   # Place Pokemon in the set
        ...
        if curpkmn || heldpkmn
          commands = [_INTL("MOVER"), _INTL("DATOS"), _INTL("SACAR")]
          if curpkmn && heldpkmn
            commands[0] = _INTL("CAMBIAR")
          elsif heldpkmn
          ...
          if !heldpkmn && curpkmn
            commands[cmdReplace = commands.length] = _INTL("REEMPLAZAR")
          end
          commands.push(_INTL("CANCELAR"))
          choice = @scene.pbShowCommands(
            _INTL("¿Qué debería hacer con este {1}?", heldpkmn ? heldpkmn.name : curpkmn.name),
            commands
          )
          if choice == 0
            if heldpkmn
            ...
            if pbBoxesFull?
              @scene.pbDisplay(_INTL("Todas las cajas están llenas."))
            elsif heldpkmn
            ...
            if pos
              ...
              if newpkmn
                if (newpkmn.shadowPokemon?) == (curpkmn.shadowPokemon?)
                  ...
                  @scene.pbDisplay(_INTL("Ese Pokémon no se puede dejar aquí."))
                end
              end
            end
          end
      ...
      when 2   # Choose a Pokemon
        ...
        if heldpkmn
          @scene.pbDisplay(_INTL("¡Llevas un Pokémon!"))
        else
          break if !@scene.pbConfirm(_INTL("¿Continuar editando los sets?"))
        end
      end
    end
    if pbCheckPurify
  ...
  def pbDisplay(msg)
  msg = PTBR_TEXT.t(msg) if msg.is_a?(String)
    @scene.pbDisplay(msg)
  end
  ...
  def pbDoPurify
    ...
    purifiables.length.times do |i|
      ...
      if (i + 1) != purifiables.length
        @scene.pbDisplay(_INTL("¡Ya hay otro Pokémon que está listo para abrir su corazón!") + "\1")
        if !@scene.pbConfirm(_INTL("¿Quieres intercambiar los sets?"))
          @scene.pbCloseSet
          break
        end
      end
  ...
  def pbStartPurify
    ...
    loop do
      ...
      if set < 0
        break if !@scene.pbConfirm(_INTL("¿Continuar viendo hologramas?"))
      else
        chamber.currentSet = set
        cmd = @scene.pbShowCommands(_INTL("¿Qué quieres hacer?"),
                                    [_INTL("EDITAR"), _INTL("CAMBIAR"), _INTL("CANCELAR")])
        case cmd
        when 0   # edit
...
class PurifyChamberSetView < Sprite
  ...
  def refresh
    ...
    if pkmn
      ...
      textpos.push([_INTL("{1}  Nv.{2}  {3}", pkmn.name, pkmn.level, type_string),
...
#   "name"      => _INTL("Cámara de Purificación"),
#   "order"     => 30,
#   "condition" => proc { next $player.seen_purify_chamber },
#   "effect"    => proc { |menu|
#     pbMessage("\\se[PC access]" + _INTL("Has accedido a la Cámara de Purificación."))
#     pbPurifyChamber
#     next false
#   }
# })