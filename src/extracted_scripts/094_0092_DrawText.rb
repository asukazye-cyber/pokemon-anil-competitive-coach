...
#  end
  ...
  while position < textchars.length
    ...
    texty = (lineheight * y) + yDst + yStart + AnilTexto::OFFSET_Y   # TEXT OFFSET (era - 2)
...
def pbDrawTextPositions(bitmap, textpos)
  textpos.each { |pos| pos[0] = PTBR_TEXT.t(pos[0]) if pos.is_a?(Array) && pos[0].is_a?(String) } if textpos.is_a?(Array) rescue nil
  textpos.each do |i|
    text = i[0] 
    text ||= ""
