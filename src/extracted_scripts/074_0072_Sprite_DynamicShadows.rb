    ...
    if @tile_id != @character.tile_id ||
      ...
      if @tile_id >= 384
        ...
        if File.exist?(skin_path + ".png") || File.exist?(skin_path + ".PNG")
          @chbitmap = AnimatedBitmap.new(skin_path, @character.character_hue)
...
class Spriteset_Map
  ...
  def initialize(map = nil)
    ...
    global = ($scene.spritesetGlobal if $scene.is_a?(Scene_Map) rescue nil)
    if global && global.playersprite
      global.playersprite.setShadows(map, @shadows)
    end
  end
end
