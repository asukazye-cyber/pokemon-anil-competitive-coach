...
def give_vial
  return if has_player_pokevial?
  pbGetKeyItem("VIAL_key")
  pbReceiveItem(:VIAL)
  init_pokevial
end
...
def lock_vial
  ...
  pbMessage(_INTL("El Pokévial ha sido bloqueado."))
end

def unlock_vial
  return unless ensure_pokevial_initialized
  $PokemonGlobal.vial_locked = false
  pbMessage(_INTL("El Pokévial ha sido desbloqueado."))
end
...
def show_message_pokevial
  if $PokemonGlobal.vial_charges <= 0
    pbMessage(_INTL("El Pokévial está vacío. Recárgalo en el Centro Pokémon."))
    return false
  end
...
def use_pokevial
  ...
  if $PokemonGlobal.vial_locked
    pbMessage(_INTL("El Pokévial está bloqueado."))
    return false
  end
...
def heal_party_with_pokevial
  $player.heal_party
  pbMEPlay("Pokemon Healing", 80, 100)
  ...
  $bag.replace_registered(:VIAL, :EMPTYVIAL) if $PokemonGlobal.vial_charges <= 0
end

def recharge_vial
    return unless ensure_pokevial_initialized
    $PokemonGlobal.vial_charges = $PokemonGlobal.max_vial_charges
    pbMessage(_INTL("¡Tu Pokévial ha sido recargado!")) if !INFINITE_POKEVIAL
    $bag.replace_item(:EMPTYVIAL,:VIAL) if $bag.has?(:EMPTYVIAL)
    $bag.replace_registered(:EMPTYVIAL,:VIAL) if $bag.has?(:VIAL)
end
...
def add_new_vial_charge
    ...
    pbMessage(_INTL("¡Ahora tu Pokévial puede almacenar {1} carga#{$PokemonGlobal.max_vial_charges > 1 ? 's' : ''}!",$PokemonGlobal.max_vial_charges))
end
...
def remove_vial_charge
  ...
  if $PokemonGlobal.max_vial_charges > 1
    ...
    pbMessage(_INTL("¡Ahora tu Pokévial puede almacenar {1} carga#{$PokemonGlobal.max_vial_charges > 1 ? 's' : ''}!", 
                           $PokemonGlobal.max_vial_charges))
  end
end
