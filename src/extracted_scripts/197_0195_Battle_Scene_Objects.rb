...
class Battle::Scene::PokemonDataBox < Sprite
  ...
  def update_positions
    if (@selected == 1 || @selected == 2) && BOBBING_DURATION
      bob_delta = System.real_uptime % BOBBING_DURATION
      bob_frame = (4 * bob_delta / BOBBING_DURATION).floor
      new_y = @spriteY
      case bob_frame
      when 1 then new_y = @spriteY - 2
      when 3 then new_y = @spriteY + 2
      end
      self.x = @spriteX
      self.y = new_y
    else
      self.x = @spriteX if self.x != @spriteX
      self.y = @spriteY if self.y != @spriteY
    end
  end
  ...
  def update
    ...
    update_positions
    # Substitui pbUpdateSpriteHash(@sprites) — direto, sem type checks
    @hpNumbers.update if @hpNumbers && !@hpNumbers.disposed?
    @hpPercent.update if @hpPercent && !@hpPercent.disposed?
    @hpBar.update     if @hpBar     && !@hpBar.disposed?
    @expBar.update    if @expBar    && !@expBar.disposed?
  end
end
...
class Battle::Scene::BattlerSprite < RPG::Sprite
  ...
  def update
    ...
    # Rock egg gently side to side
    if @is_egg
      self.angle = 4 * Math.sin(System.uptime * 2 * Math::PI / 1.5)
    else
      self.angle = 0
    end
    self.x       = self.x
