...
def pbBattleAnimation(bgm = nil, battletype = 0, foe = nil)
  ...
  handled = false
  # Diagnostico: qual animacao de introducao foi escolhida, e porque.
  # (Data/anil_debug.txt liga; grava em Data/skin_sprite_log.txt.)
  # ⚠️ UMA SO ABERTURA DE FICHEIRO, E DEPOIS DO CICLO.
  #
  # Isto abria o skin_sprite_log.txt uma vez por CANDIDATA, dentro da entrada em
  # batalha. Um File.open por volta do ciclo e I/O sincrono no meio da
  # transicao — no Android sao segundos de ecra preto, e o ficheiro chegou aos
  # 2,1 MB. As linhas juntam-se em memoria e vao ao disco de uma vez.
  anil_dbg    = (anil_diagnostico_ligado? rescue false)
  anil_linhas = []
  if anil_dbg
    anil_linhas << "INTRO battletype=#{battletype} foes=#{(foe.length rescue '?')} "                    "local=#{location} foe_tr_type=#{(foe[0].trainer_type rescue nil).inspect} "                    "foe_skin=#{(foe[0].multiplayer_skin rescue nil).inspect}"
  end
  SpecialBattleIntroAnimations.each do |name, priority, condition, animation|
    passou = (condition.call(battletype, foe, location) rescue false)
    anil_linhas << "  candidata #{name.inspect} (p#{priority}) => #{passou}" if anil_dbg
    next if !passou
    animation.call(viewport, battletype, foe, location)
    handled = true
    break
  end
  anil_linhas << "  NENHUMA passou — animacao padrao (bolas)" if anil_dbg && !handled
  if anil_dbg && !anil_linhas.empty?
    begin
      carimbo = Time.now.strftime("%H:%M:%S")
      File.open("Data/skin_sprite_log.txt", "a") do |f|
        anil_linhas.each { |l| f.puts("[#{carimbo}] #{l}") }
      end
    rescue
    end
  end
...
SpecialBattleIntroAnimations.register("alternate_vs_trainer_animation", 50,   # Priority 50
  proc { |battle_type, foe, location|   # Condition
    ...
    if ctx && ctx.mode == :pvp
      next true # Sempre ativa a animação de VS no PVP!
    end
    tr_type = foe[0].trainer_type
  ...
  proc { |viewport, battle_type, foe, location|   # Animation
    # Resolvers locais via procs para suportar skins dinâmicas/PVP
    pbResolveVSTrainerSprite = proc { |trainer_obj, is_player|
      # Diagnostico (Data/anil_debug.txt liga; grava em Data/skin_sprite_log.txt).
      begin
        if anil_diagnostico_ligado?
          File.open("Data/skin_sprite_log.txt", "a") do |f|
            f.puts("[#{Time.now.strftime('%H:%M:%S')}] VS_SPRITE is_player=#{is_player} " \
                   "classe=#{trainer_obj.class} " \
                   "responde_skin=#{trainer_obj.respond_to?(:multiplayer_skin)} " \
                   "skin=#{(trainer_obj.multiplayer_skin rescue '(erro)').inspect} " \
                   "tr_type=#{(trainer_obj.trainer_type rescue nil).inspect}")
          end
        end
      rescue
      end
      if trainer_obj && trainer_obj.respond_to?(:multiplayer_skin) && trainer_obj.multiplayer_skin && !trainer_obj.multiplayer_skin.empty?
        char_name = trainer_obj.multiplayer_skin
        sprite_name = ""
        if defined?(AnilLanRework) && AnilLanRework.respond_to?(:trainer_sprite_name_for_character)
          sprite_name = AnilLanRework.trainer_sprite_name_for_character(char_name) rescue ""
        end
        begin
          if anil_diagnostico_ligado?
            File.open("Data/skin_sprite_log.txt", "a") do |f|
              f.puts("[#{Time.now.strftime('%H:%M:%S')}]   -> sprite_name=#{sprite_name.inspect} " \
                     "vs_existe=#{!!pbResolveBitmap("Graphics/Transitions/vsTrainer_#{sprite_name}")} " \
                     "front_existe=#{!!pbResolveBitmap("Graphics/Trainers/#{sprite_name}")}")
            end
          end
        rescue
        end
        unless sprite_name.empty?
          vs_img = "vsTrainer_#{sprite_name}"
          if pbResolveBitmap("Graphics/Transitions/#{vs_img}")
            next RPG::Cache.transition(vs_img)
          end
          front_img = "Graphics/Trainers/#{sprite_name}"
          if pbResolveBitmap(front_img)
            next RPG::Cache.load_bitmap("Graphics/Trainers/", sprite_name)
          end
        end
      end

      if is_player
        player_tr_type = $player.trainer_type rescue nil
        player_tr_type = GameData::TrainerType.keys.first if !player_tr_type
        outfit = $player.outfit rescue 0
        player_graphic = sprintf("vsTrainer_%s_%d", player_tr_type.to_s, outfit) rescue nil
        if player_graphic && pbResolveBitmap("Graphics/Transitions/" + player_graphic)
          next RPG::Cache.transition(player_graphic)
        end
        player_graphic = sprintf("vsTrainer_%s", player_tr_type.to_s) rescue nil
        if player_graphic && pbResolveBitmap("Graphics/Transitions/" + player_graphic)
          next RPG::Cache.transition(player_graphic)
        end
        front_img = GameData::TrainerType.front_sprite_filename(player_tr_type) rescue nil
        if front_img && pbResolveBitmap(front_img)
          filename = File.basename(front_img, ".*")
          next RPG::Cache.load_bitmap("Graphics/Trainers/", filename)
        end
      else
        tr_type = trainer_obj.trainer_type rescue nil
        tr_type = GameData::TrainerType.keys.first if !tr_type
        trainer_graphic = sprintf("vsTrainer_%s", tr_type.to_s) rescue nil
        if trainer_graphic && pbResolveBitmap("Graphics/Transitions/" + trainer_graphic)
          next RPG::Cache.transition(trainer_graphic)
        end
        front_img = GameData::TrainerType.front_sprite_filename(tr_type) rescue nil
        if front_img && pbResolveBitmap(front_img)
          filename = File.basename(front_img, ".*")
          next RPG::Cache.load_bitmap("Graphics/Trainers/", filename)
        end
    ...
    pbResolveVSTrainerBar = proc { |trainer_obj, is_player|
      if trainer_obj && trainer_obj.respond_to?(:multiplayer_skin) && trainer_obj.multiplayer_skin && !trainer_obj.multiplayer_skin.empty?
        char_name = trainer_obj.multiplayer_skin
        sprite_name = ""
        if defined?(AnilLanRework) && AnilLanRework.respond_to?(:trainer_sprite_name_for_character)
        ...
        unless sprite_name.empty?
          vs_bar = "vsBar_#{sprite_name}"
          if pbResolveBitmap("Graphics/Transitions/#{vs_bar}")
            next RPG::Cache.transition(vs_bar)
          end
        end
      end
      ...
      if is_player
        ...
        player_tr_type = GameData::TrainerType.keys.first if !player_tr_type
        outfit = $player.outfit rescue 0
        player_bar = sprintf("vsBar_%s_%d", player_tr_type.to_s, outfit) rescue nil
        if player_bar && pbResolveBitmap("Graphics/Transitions/" + player_bar)
          next RPG::Cache.transition(player_bar)
        end
        player_bar = sprintf("vsBar_%s", player_tr_type.to_s) rescue nil
        if player_bar && pbResolveBitmap("Graphics/Transitions/" + player_bar)
          next RPG::Cache.transition(player_bar)
        end
        next RPG::Cache.transition("vsBar_POKEMONTRAINER_Red") rescue nil
      else
        tr_type = trainer_obj.trainer_type rescue nil
        tr_type = GameData::TrainerType.keys.first if !tr_type
        trainer_bar = sprintf("vsBar_%s", tr_type.to_s) rescue nil
        if trainer_bar && pbResolveBitmap("Graphics/Transitions/" + trainer_bar)
          next RPG::Cache.transition(trainer_bar)
        end
        next RPG::Cache.transition("vsBar_POKEMONTRAINER_Red") rescue nil
      end
    }

    # Carrega bitmaps dinamicamente usando os resolvers
    bar1_bitmap = pbResolveVSTrainerBar.call($player, true)
    bar2_bitmap = pbResolveVSTrainerBar.call(foe[0], false)
    player_bitmap = pbResolveVSTrainerSprite.call($player, true)
    trainer_bitmap = pbResolveVSTrainerSprite.call(foe[0], false)
    ...
    bar1.bitmap = bar1_bitmap
    bar1.x      = -xoffset
    bar2 = Sprite.new(viewopp)
    bar2.bitmap = bar2_bitmap
    bar2.x      = xoffset
    ...
    player.bitmap = player_bitmap
    player.x      = -xoffset
    trainer = Sprite.new(viewopp)
    trainer.bitmap = trainer_bitmap
    trainer.x      = xoffset
