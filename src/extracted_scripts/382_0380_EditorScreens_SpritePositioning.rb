...
class SpritePositioner
  ...
  def pbClose
    if @metricsChanged && pbConfirmMessage(_INTL("Algunas métricas se han editado. ¿Guardar cambios?"))
      pbSaveMetrics
      @metricsChanged = false
    else
  ...
  def pbChangeSpecies(species, form, shiny = false)
    @species = species
    @form = form
    species_data = GameData::Species.get_species_form(@species, @form)
    return if !species_data
    @sprites["pokemon_0"].setSpeciesBitmap(@species, 0, @form, shiny, false, true)
    @sprites["pokemon_1"].setSpeciesBitmap(@species, 0, @form, shiny)
  ...
  def pbShadowSize
    ...
    if pbResolveBitmap(sprintf("Graphics/Pokemon/Shadow/%s_%d", metrics_data.species, metrics_data.form)) ||
      ...
      pbMessage("Esta especie tiene su propio sprite oscuro en la carpeta Graphics/Pokemon/Shadow/. Las métricas del tamaño de la sombra no se pueden editar.")
      return false
    end
