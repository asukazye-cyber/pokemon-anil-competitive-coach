...
class Battle::Move::SwitchOutTargetStatusMove < Battle::Move
  ...
  def pbSwitchOutTargetEffect(user, targets, numHits, switched_battlers)
    ...
    targets.each do |b|
      ...
      @battle.pbDisplay(_INTL("¡{1} fue arrastrado al combate!", b.pbThis))
...
class Battle::Move::BindTarget < Battle::Move
  def pbEffectAgainstTarget(user, target)
    ...
    when :WHIRLPOOL
      msg = _INTL("¡{1} fue atrapado en el torbellino!", target.pbThis)
    when :WRAP
      msg = _INTL("¡{1} fue atrapado por {2}!", target.pbThis, user.pbThis(true))
    end
...
class Battle::Move::TrapTargetInBattle < Battle::Move
  ...
  def pbFailsAgainstTarget?(user, target, show_message)
    ...
    if Settings::MORE_TYPE_EFFECTS && target.pbHasType?(:GHOST)
      @battle.pbDisplay(_INTL("No afecta al {1}...", target.pbThis(true))) if show_message
      return true
    end
    return false
  end
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("¡{1} no puede escapar!", target.pbThis))
  end
...
class Battle::Move::TrapTargetInBattleLowerTargetDefSpDef1EachTurn < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("¡{1} no puede escapar debido a {2}!", target.pbThis, @name))
  end
end
...
class Battle::Move::UsedAfterUserTakesPhysicalDamage < Battle::Move
  def pbDisplayChargeMessage(user)
    ...
    @battle.pbDisplay(_INTL("¡{1} preparó una trampa!", user.pbThis))
  end
...
class Battle::Move::TargetActsNext < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("¡{1} ha decidido aceptar la oportunidad!", target.pbThis))
  end
end
...
class Battle::Move::TargetActsLast < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("¡{1} ha retrasado su turno!", target.pbThis))
  end
end
...
#===============================================================================
# If Misty Terrain applies, priority is increased by 1. (Bruma Explosiva)
#===============================================================================
...
class Battle::Move::LowerPPOfTargetLastMoveBy3 < Battle::Move
  def pbAdditionalEffect(user, target)
    ...
    @battle.pbDisplay(_INTL("¡El movimiento {2} de {1} ha perdido 3 PP!",
...
class Battle::Move::LowerPPOfTargetLastMoveBy4 < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    target.pbSetPP(last_move, last_move.pp - reduction)
    @battle.pbDisplay(_INTL("¡El movimiento {2} de {1} ha perdido 4 PP!",
                            target.pbThis(true), last_move.name, reduction))
  end
end
...
#        "UseLastMoveUsedByTarget",                    # Mirror Move   # See above
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("¡{1} sufrió los efectos de Otra Vez!", target.pbThis))
    target.pbItemStatusCureCheck
  end
end
    ...
    if Settings::MECHANICS_GENERATION >= 6 && target.hasActiveAbility?(:OBLIVIOUS) &&
      ...
      if show_message
        ...
        if Battle::Scene::USE_ABILITY_SPLASH
          ...
          @battle.pbDisplay(_INTL("¡Pero ha fallado debido a {2} de {1}!",
