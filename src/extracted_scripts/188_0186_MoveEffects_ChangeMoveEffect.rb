...
class Battle::Move::RedirectAllMovesToUser < Battle::Move
  def pbEffectGeneral(user)
    ...
    @battle.pbDisplay(_INTL("¡{1} se ha convertido en el centro de atención!", user.pbThis))
  end
end
...
class Battle::Move::RedirectAllMovesToTarget < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("¡{1} se ha convertido en el centro de atención!", target.pbThis))
  end
end
...
class Battle::Move::CurseTargetOrLowerUserSpd1RaiseUserAtkDef1 < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    return if !user.pbHasType?(:GHOST)
    # Ghost effect
    @battle.pbDisplay(_INTL("¡{1} reduce sus PS y maldice al {2} enemigo!", user.pbThis, target.pbThis(true)))
...
class Battle::Move::TargetNextFireMoveDamagesTarget < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("¡{1} está cubierto de polvo!", user.pbThis))
  end
end
...
class Battle::Move::PowerUpAllyMove < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    ...
    @battle.pbDisplay(_INTL("¡{1} está listo para ayudar a {2}!", user.pbThis, target.pbThis(true)))
  end
end
...
class Battle::Move::UserAddStockpileRaiseDefSpDef1 < Battle::Move
  ...
  def pbMoveFailed?(user, targets)
    if user.effects[PBEffects::Stockpile] >= 3
      @battle.pbDisplay(_INTL("¡{1} no puede reservar más!", user.pbThis))
      return true
    end
    return false
  end
  ...
  def pbEffectGeneral(user)
    ...
    @battle.pbDisplay(_INTL("¡{1} reservó {2}!",
...
class Battle::Move::PowerDependsOnUserStockpile < Battle::Move
  def pbMoveFailed?(user, targets)
    if user.effects[PBEffects::Stockpile] == 0
      @battle.pbDisplay(_INTL("¡Pero no pudo escupir nada!"))
      return true
    end
    return false
  end
...
class Battle::Move::HealUserDependingOnUserStockpile < Battle::Move
  ...
  def pbMoveFailed?(user, targets)
    if user.effects[PBEffects::Stockpile] == 0
      @battle.pbDisplay(_INTL("¡Pero no pudo tragar nada!"))
      return true
    end
    if !user.canHeal? &&
       user.effects[PBEffects::StockpileDef] == 0 &&
...
class Battle::Move::UseLastMoveUsedByTarget < Battle::Move
  ...
  def pbFailsAgainstTarget?(user, target, show_message)
    if !target.lastRegularMoveUsed ||
      ...
      @battle.pbDisplay(_INTL("¡El Movimiento Espejo ha fallado!")) if show_message
      return true
    end
    return false
  end
...
class Battle::Move::UseMoveDependingOnEnvironment < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    @battle.pbDisplay(_INTL("¡{1} se convirtió en {2}!", @name, GameData::Move.get(@npMove).name))
...
class Battle::Move::BounceBackProblemCausingStatusMoves < Battle::Move
  def pbEffectGeneral(user)
    ...
    @battle.pbDisplay(_INTL("¡{1} se cubrió con Capa Mágica!", user.pbThis))
  end
end
...
class Battle::Move::StealAndUseBeneficialStatusMove < Battle::Move
  def pbEffectGeneral(user)
    ...
    @battle.pbDisplay(_INTL("¡{1} espera a que su rival haga algún movimiento!", user.pbThis))
  end
end
...
class Battle::Move::ReplaceMoveThisBattleWithTargetLastMoveUsed < Battle::Move
  ...
  def pbEffectAgainstTarget(user, target)
    user.eachMoveWithIndex do |m, i|
      ...
      @battle.pbDisplay(_INTL("¡{1} ha aprendido {2}!", user.pbThis, newMove.name))
