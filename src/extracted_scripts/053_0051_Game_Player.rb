...
class Game_Player < Game_Character
  ...
  def set_movement_type(type)
    ...
    # Aplicar skin multiplayer se existir e não estiver em veículo
    # Isso garante que a skin persista ao andar e correr
    is_vehicle = [:diving, :surfing, :bicycle].any? { |v| type.to_s.include?(v.to_s) } ||
                 $PokemonGlobal&.surfing || $PokemonGlobal&.diving || $PokemonGlobal&.bicycle
  ...
  # isn't moving.
  def refresh_charset
    # Prioridade para a skin escolhida no menu multiplayer
    if $player&.multiplayer_skin && !$PokemonGlobal&.diving && !$PokemonGlobal&.surfing && !$PokemonGlobal&.bicycle
      @character_name = $player.multiplayer_skin
      return
    end
    meta = GameData::PlayerMetadata.get($player&.character_ID || 1)
  ...
  def pbTriggeredTrainerEvents(triggers, checkIfRunning = true, trainer_only = false)
    ...
    if defined?(AnilLanRework) && AnilLanRework.peer_interaction_suppressed?
      return result
    end
    # All event loops
    $game_map.events.each_value do |event|
...
def pbGetPlayerCharset(charset, trainer = nil, force = false)
  ...
  # Prioridade para a skin multiplayer se disponível e não estiver em veículo
  if trainer&.respond_to?(:multiplayer_skin) && trainer.multiplayer_skin
    # No menu de Load ou se não estiver em nenhum veículo, usa a skin
    if (defined?(PokemonLoad_Scene) && $scene.is_a?(PokemonLoad_Scene)) || 
       (!$PokemonGlobal || (!$PokemonGlobal.bicycle && !$PokemonGlobal.surfing && !$PokemonGlobal.diving))
      resolved_skin = pbResolvePlayerCharsetName(trainer.multiplayer_skin)
      return resolved_skin if resolved_skin
    end
  end
  ...
  resolved = pbResolvePlayerCharsetName(ret)
  return resolved if resolved
  return ret
end

def pbResolvePlayerCharsetName(name)
  raw_name = name.to_s.dup
  return nil if raw_name.empty?
  begin
    raw_name = raw_name.encode(Encoding::UTF_8, invalid: :replace, undef: :replace, replace: "")
  rescue
  end
  raw_name = raw_name.delete("\u0000")
  raw_name = raw_name.gsub(/[\r\n\t]/, "")
  raw_name = raw_name.strip
  raw_name = raw_name.sub(/\A["']+/, "").sub(/["']+\z/, "")
  raw_name = raw_name.sub(/\A(?:Graphics\/Characters\/|Graphics\/Skins\/)/i, "")
  raw_name = raw_name.sub(/\.(png|jpg|jpeg|gif|bmp)\z/i, "")
  return nil if raw_name.empty?
  candidates = []
  candidates << raw_name
  candidates << "Graphics/Characters/#{raw_name}"
  candidates << "Graphics/Skins/#{raw_name}"
  candidates.uniq.each do |candidate|
    resolved = pbResolveBitmap(candidate) rescue nil
    next if !resolved
    return candidate.sub(/\A(?:Graphics\/Characters\/|Graphics\/Skins\/)/i, "")
  end
  nil
end
