module GameData
  class GrowthRate
    ...
    def minimum_exp_for_level(level)
      return ArgumentError.new("El nivel #{level} no es válido.") if !level || level <= 0
      level = [level, GrowthRate.max_level].min
      return @exp_values[level] if level < @exp_values.length
      raise "No se ha definido una fórmula de exp. para el tipo de crecimiento #{name}" if !@exp_formula
    ...
    def level_from_exp(exp)
      return ArgumentError.new("La cantidad de experiencia #{level} no es válida.") if !exp || exp < 0
