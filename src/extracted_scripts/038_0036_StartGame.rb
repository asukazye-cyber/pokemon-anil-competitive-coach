...
module Game
  ...
  def self.initialize
    ...
    # pbLoadBattleAnimations # Lazy load: carregado apenas sob demanda para acelerar o boot
    GameData.load_all
  ...
  def self.start_new
    ...
    if defined?(AnilLanRework)
      AnilLanRework.request_map_graphics_refresh!("start_new_game", 2)
    end
  end
  ...
  def self.load(save_data)
    ...
    $scene = Scene_Map.new
    if defined?(AnilLanRework)
      AnilLanRework.request_map_graphics_refresh!("load_game", 2)
    end
  end
