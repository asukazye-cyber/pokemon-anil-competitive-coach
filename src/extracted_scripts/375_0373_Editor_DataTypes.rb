...
module UndefinedProperty
  def self.set(_settingname, oldsetting)
    pbMessage(_INTL("Esta propiedad no se puede editar aquí en este momento"))
    return oldsetting
  end
...
module ReadOnlyProperty
  def self.set(_settingname, oldsetting)
    pbMessage(_INTL("Esta propiedad no se puede editar."))
    return oldsetting
  end
...
class UIntProperty
  ...
  def set(settingname, oldsetting)
    ...
    return pbMessageChooseNumber(_INTL("Elige el valor para {1}.", settingname), params)
  end
...
class LimitProperty
  ...
  def set(settingname, oldsetting)
    ...
    return pbMessageChooseNumber(_INTL("Elige el valor para {1} (0-{2}).", settingname, @maxvalue), params)
  end
...
class LimitProperty2
  ...
  def set(settingname, oldsetting)
    ...
    ret = pbMessageChooseNumber(_INTL("Elige el valor para {1} (0-{2}).", settingname, @maxvalue), params)
...
class NonzeroLimitProperty
  ...
  def set(settingname, oldsetting)
    ...
    params.setDefaultValue(oldsetting)
    return pbMessageChooseNumber(_INTL("Elige un valor para {1}.", settingname), params)
  end
...
module BooleanProperty
  def self.set(settingname, _oldsetting)
    return pbConfirmMessage(_INTL("¿Permitir la configuración de {1}?", settingname)) ? true : false
  end
...
module BooleanProperty2
  def self.set(_settingname, _oldsetting)
    ret = pbShowCommands(nil, [_INTL("Verdadero"), _INTL("Falso")], -1)
  ...
  def self.format(value)
    return _INTL("Verdadero") if value
    return (value.nil?) ? "-" : _INTL("Falso")
  end
end
...
module StringProperty
  def self.set(settingname, oldsetting)
    return pbMessageFreeText(_INTL("Elegir el valor para {1}.", settingname),
...
class EnumProperty
  ...
  def set(settingname, oldsetting)
    ...
    cmd = pbMessage(_INTL("Elige un valor para {1}.", settingname), commands, -1)
...
class EnumProperty2
  ...
  def set(settingname, oldsetting)
    ...
    cmd = pbMessage(_INTL("Elige un valor para {1}.", settingname), commands, -1, nil, oldsetting)
    return oldsetting if cmd < 0
    return cmd
  end
...
class StringListProperty
  def self.set(_setting_name, old_setting)
    ...
    real_cmds = []
    real_cmds.push([_INTL("[AÑADIR VALOR]"), -1])
    ...
    loop do
      ...
      if cmd >= 0   # Chose a value
        ...
        if entry[1] == -1   # Add new value
          new_value = pbMessageFreeText(_INTL("Introduce el nuevo valor."),
          ...
          case pbMessage("\\ts[]" + _INTL("¿Qué hacer con este valor?"),
                         [_INTL("Editar"), _INTL("Eliminar"), _INTL("Cancelar")], 3)
          when 0   # Edit
            new_value = pbMessageFreeText(_INTL("Introduce el nuevo valor."),
        ...
        case pbMessage(_INTL("¿Guardar cambios?"), [_INTL("Sí"), _INTL("No"), _INTL("Cancelar")], 3)
        when 0
...
class GameDataProperty
  def initialize(value)
    raise _INTL("No se encuentra la clase {1} en el módulo GameData.", value.to_s) if !GameData.const_defined?(value.to_sym)
...
module TypesProperty
  def self.set(_settingname, oldsetting)
    ...
    loop do
      cmds = []
      2.times { |i| cmds.push(_INTL("Tipo {1} : {2}", i, ret[i] || "-")) }
      index = pbMessage(_INTL("Elige los tipos de esta especie."), cmds, -1)
      break if index < 0
    ...
    return ret if ret != oldsetting.compact && pbConfirmMessage(_INTL("¿Aplicar cambios?"))
    return oldsetting
  end
...
module GenderProperty
  def self.set(_settingname, _oldsetting)
    ret = pbShowCommands(nil, [_INTL("Macho"), _INTL("Hembra")], -1)
    return (ret >= 0) ? ret : nil
  end
  ...
  def self.format(value)
    return "-" if !value
    return _INTL("Macho") if value == 0
    return _INTL("Hembra") if value == 1
    return "-"
  end
end
  ...
  def set(settingname, oldsetting)
    ...
    GameData::Stat.each_main do |s|
      ...
      properties[s.pbs_order] = [s.name, LimitProperty2.new(@limit),
                                 _INTL("Valores individuales para la estadística {1} del Pokémon (0-{2}).", s.name, @limit)]
...
class EVsProperty
  ...
  def set(settingname, oldsetting)
    ...
    GameData::Stat.each_main do |s|
      oldsetting[s.pbs_order] = 0 if !oldsetting[s.pbs_order]
      properties[s.pbs_order] = [s.name, LimitProperty2.new(@limit),
                                 _INTL("Puntos de esfuerzo para la estadística {1} del Pokémon (0-{2}).", s.name, @limit)]
      data[s.pbs_order] = oldsetting[s.id]
      stat_ids[s.pbs_order] = s.id
    end
    loop do
      ...
      pbMessage(_INTL("Los PE (EVs) totales ({1}) son más que los permitidos ({2}). Por favor, redúcelos.", evtotal, Pokemon::EV_LIMIT))
    end
    ret = {}
    stat_ids.each_with_index { |s, i| ret[s] = data[i] || 0 }
    return ret
...
module MapSizeProperty
  def self.set(settingname, oldsetting)
    oldsetting = [0, ""] if !oldsetting
    properties = [
      [_INTL("Ancho"),         NonzeroLimitProperty.new(30), _INTL("El ancho del mapa en cuadrados del Mapa de la Región.")],
      [_INTL("Cuadrados Válidos"), StringProperty, _INTL("Una serie de 1s y 0s que marcan qué cuadrados forman parte o no de este mapa (1=sí, 0=no).")]
    ]
...
def chooseMapPoint(map, rgnmap = false)
  ...
  title = Window_UnformattedTextPokemon.newWithSize(
    _INTL("Haz click en un punto del mapa."), 0, Graphics.height - 64, Graphics.width, 64, viewport
  )
  title.z = 2
  if rgnmap
...
module MapCoordsFacingProperty
  def self.set(settingname, oldsetting)
    ...
    if chosenmap >= 0
      mappoint = chooseMapPoint(chosenmap)
      if mappoint
        facing = pbMessage(_INTL("Elige la dirección a la que mirar."),
...
module RegionMapCoordsProperty
  def self.set(_settingname, oldsetting)
    ...
    case regions.length
    when 0
      pbMessage(_INTL("No se han definido mapas de la región."))
      return oldsetting
    when 1
      ...
      selcmd = pbMessage(_INTL("Elige un mapa de la región."), cmds, -1)
...
module WeatherEffectProperty
  def self.set(_settingname, oldsetting)
    ...
    cmd = pbMessage(_INTL("Elige un clima."), options, -1, nil, default)
    return nil if cmd < 0 || ids[cmd] == :None
    params = ChooseNumberParams.new
    params.setRange(0, 100)
    params.setDefaultValue(oldsetting[1])
    number = pbMessageChooseNumber(_INTL("Elige la probabilidad del clima."), params)
...
module PocketProperty
  def self.set(_settingname, oldsetting)
    ...
    cmd = pbMessage(_INTL("Elige un bolsillo para este objeto."), commands, -1)
...
module BaseStatsProperty
  def self.set(settingname, oldsetting)
    ...
    GameData::Stat.each_main do |s|
      next if s.pbs_order < 0
      properties[s.pbs_order] = [_INTL("{1} base", s.name), NonzeroLimitProperty.new(255),
                                 _INTL("Estadística de {1} base del Pokémon.", s.name)]
...
module EffortValuesProperty
  def self.set(settingname, oldsetting)
    ...
    GameData::Stat.each_main do |s|
      next if s.pbs_order < 0
      properties[s.pbs_order] = [_INTL("EVs de {1}", s.name), LimitProperty.new(255),
                                 _INTL("Número de Puntos de Esfuerzo (EVs) de {1} que se ganan del Pokémon.", s.name)]
      data[s.pbs_order] = 0
...
class GameDataPoolProperty
  ...
  def set(setting_name, old_setting)
    ...
    values = []
    values.push([nil, _INTL("[AÑADIR VALOR]")])   # Value ID, name
      ...
      when 0
        if cmd[1] >= 0   # Chose an entry
          ...
          if entry[0].nil?   # Add new value
            ...
            case pbMessage("\\ts[]" + _INTL("¿Qué hacer con este valor?"),
                           [_INTL("Cambiar valor"), _INTL("Eliminar"), _INTL("Cancelar")], 3)
            when 0   # Change value
          ...
          case pbMessage(_INTL("¿Aplicar cambios?"),
    ...
    loop do
      if refreshlist
        ...
        realcmds.each_with_index do |entry, i|
          if entry[0] == -1
            commands.push(_INTL("[AÑADIR VALOR]"))
          else
      ...
      when 0
        if cmd[1] >= 0   # Chose an entry
          ...
          if entry[0] == -1   # Add new move
            ...
            params.setCancelValue(-1)
            newlevel = pbMessageChooseNumber(_INTL("Elige un nivel."), params)
            if newlevel >= 0
            ...
            case pbMessage("\\ts[]" + _INTL("¿Qué hacer con este movimiento?"),
                           [_INTL("Cambiar nivel"), _INTL("Cambiar movimiento"), _INTL("Eliminar"), _INTL("Cancelar")], 4)
            when 0   # Change level
              params = ChooseNumberParams.new
              params.setRange(0, GameData::GrowthRate.max_level)
              params.setDefaultValue(entry[0])
              newlevel = pbMessageChooseNumber(_INTL("Elige un nuevo nivel."), params)
...
class EvolutionsProperty
  ...
  def edit_parameter(evo_method, value = nil)
    ...
    when String
      ...
      params.setCancelValue(-1)
      ret = pbMessageChooseNumber(_INTL("Elige un parámetro."), params)
      ret = nil if ret < 0
    end
  ...
  def set(_settingname, oldsetting)
    ...
    loop do
      if refreshlist
        ...
        realcmds.length.times do |i|
          if realcmds[i][3] < 0
            commands.push(_INTL("[AÑADIR EVOLUCIÓN]"))
          else
            level = realcmds[i][2]
      ...
      when 0
        if cmd[1] >= 0
          ...
          if entry[3] == -1   # Add new evolution path
            pbMessage(_INTL("Elige un Pokémon para ser la evolución, un método y un parámetro."))
            newspecies = pbChooseSpeciesList
            if newspecies
              newmethodindex = pbMessage(_INTL("Elige un método de evolución."), @methods, -1)
              if newmethodindex >= 0
            ...
            case pbMessage("\\ts[]" + _INTL("¿Qué hacer con esta evolución?"),
                           [_INTL("Cambiar especie"), _INTL("Change method"),
                            _INTL("Cambiar parámetro"), _INTL("Eliminar"), _INTL("Cancelar")], 5)
            ...
            when 1   # Change method
              ...
              newmethodindex = pbMessage(_INTL("Elige un método de evolución."), @methods, -1, nil, default_index)
              if newmethodindex >= 0
                newmethod = @evo_ids[newmethodindex]
                existing_evo = -1
                realcmds.length.times do |i|
            ...
            when 2   # Change parameter
              if GameData::Evolution.get(entry[1]).parameter.nil?
                pbMessage(_INTL("Este método de evolución no usa parámetros."))
              else
...
module EncounterSlotProperty
  def self.set(setting_name, data)
    ...
    properties = [
      [_INTL("Probabilidad"), NonzeroLimitProperty.new(999),       _INTL("Probabilidad relativa para elegir este slot.")],
      [_INTL("Especie"),      SpeciesFormProperty.new(data[1]),    _INTL("La especie/forma de un Pokémon.")],
      [_INTL("Nivel mínimo"), NonzeroLimitProperty.new(max_level), _INTL("Nivel mínimo de esta especie (1-{1}).", max_level)],
      [_INTL("Nivel máximo"), NonzeroLimitProperty.new(max_level), _INTL("Nivel máximo de esta especie (1-{1}).", max_level)]
    ]
  ...
  def self.format(value)
    ...
    if species_data.form > 0
      if value[2] == value[3]
        return sprintf("%d, %s_%d (Nv.%d)", value[0],
                       species_data.real_name, species_data.form, value[2])
      end
      return sprintf("%d, %s_%d (Nv.%d-%d)", value[0],
                     species_data.real_name, species_data.form, value[2], value[3])
    end
    if value[2] == value[3]
      return sprintf("%d, %s (Nv.%d)", value[0], species_data.real_name, value[2])
    end
    return sprintf("%d, %s (Nv.%d-%d)", value[0], species_data.real_name, value[2], value[3])
  end
end
...
def pbPropertyList(title, data, properties, saveprompt = false)
  ...
  loop do
    loop do
      ...
      if Input.trigger?(Input::ACTION)
        ...
        if propobj != ReadOnlyProperty && !propobj.is_a?(ReadOnlyProperty) &&
           pbConfirmMessage(_INTL("¿Reiniciar la configuración {1}?", properties[selectedmap][0]))
    ...
    if selectedmap == -1 && saveprompt
      cmd = pbMessage(_INTL("¿Guardar cambios?"),
                      [_INTL("Sí"), _INTL("No"), _INTL("Cancelar")], 3)
      if cmd == 2
        selectedmap = list.index
      else
