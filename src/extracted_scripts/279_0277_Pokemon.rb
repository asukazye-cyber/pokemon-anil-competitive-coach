...
class Pokemon
  ...
  def super_shiny?
    if @super_shiny.nil?
      ...
      c = (a >> 16) & 0xFFFF
      d = b ^ c
      # Antes: d == 0, ou seja 1 em 65536. Uma varredura dos 5181 saves achou
      # exatamente 4 super shinies vindos desse sorteio (contra 136 gravados a
      # mao pelo painel/ovos): na pratica ninguem tirava um jogando.
      #
      # Agora ~1 em 15123 (alvo 1/15000). Como d e de 16 bits, so ele nao chega
      # nessa granularidade: d < 13 da 1/5041, e o segundo teste sobre os bits
      # altos de a corta isso em tres. Os bits altos nao entram no valor de d
      # de forma isolada, entao os dois testes sao praticamente independentes.
      #
      # d < 13 e a MESMA condicao do shiny? logo acima: isso garante que todo
      # super shiny continua sendo shiny. Se mexer em SHINY_POKEMON_CHANCE,
      # este 13 tem de acompanhar, senao a garantia cai.
      #
      # O evento de 08/08/2026 (`% 5) < 2`, para acompanhar o 33) foi encerrado
      # em 12/08/2026 e o divisor voltou ao normal.
      @super_shiny = (d < Settings::SHINY_POKEMON_CHANCE) && (((a >> 16) % 3) == 0)
    end
    return @super_shiny
  end
