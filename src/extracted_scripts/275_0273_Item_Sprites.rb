...
class ItemIconSprite < Sprite
  ...
  def dispose
    # We do NOT dispose @animbitmap since it is globally cached to prevent disk-re-reading lag!
    # @animbitmap&.dispose
    super
  end
  ...
  def item=(value)
    ...
    @animbitmap = nil
    if @item || !@blankzero
      filename = GameData::Item.icon_filename(@item)
      $item_icon_cache ||= {}
      if !$item_icon_cache[filename] || $item_icon_cache[filename].disposed?
        $item_icon_cache[filename] = AnimatedBitmap.new(filename)
      end
      @animbitmap = $item_icon_cache[filename]
