...
def pbPurify(pkmn, scene)
  ...
  scene.pbDisplay(_INTL("¡{1} abrió la puerta de su corazón!", pkmn.name))
  old_moves = []
  pkmn.moves.each { |m| old_moves.push(m.id) }
  pkmn.update_shadow_moves
  pkmn.moves.each_with_index do |m, i|
    next if m.id == old_moves[i]
    scene.pbDisplay(_INTL("¡{1} recuperó el movimiento {2}!", pkmn.name, m.name))
  end
  ...
  if pkmn.saved_exp
    ...
    curlevel = pkmn.level
    if newexp != pkmn.exp
      scene.pbDisplay(_INTL("¡{1} recuperó {2} Puntos de Exp.!", pkmn.name, newexp - pkmn.exp))
    end
  ...
  if $PokemonSystem.givenicknames == 0 &&
     scene.pbConfirm(_INTL("¿Quieres darle un mote a {1}?", pkmn.speciesName))
...
class RelicStoneScreen
  ...
  def pbDisplay(x)
  x = PTBR_TEXT.t(x) if x.is_a?(String)
    @scene.pbDisplay(x)
  end
...
def pbRelicStone
  if $player.party.none? { |pkmn| pkmn.purifiable? }
    pbMessage(_INTL("No tienes Pokémon que puedan ser purificados."))
    return
  end
  pbMessage(_INTL("¡Hay un Pokémon que podría abrir la puerta de su corazón!"))
...
class Battle
  ...
  def pbCanUseItemOnPokemon?(item, pkmn, battler, scene, showMessages = true)
    ...
    if ret && pkmn.hyper_mode && ![:JOYSCENT, :EXCITESCENT, :VIVIDSCENT].include?(item)
      scene.pbDisplay(_INTL("Este objeto no se puede usar en este Pokémon."))
      return false
    end
    return ret
  end
...
class Battle::Battler
  ...
  def pbHyperMode
    ...
    if @battle.pbRandom(p.heart_gauge) <= p.max_gauge_size / 4
      p.hyper_mode = true
      @battle.pbDisplay(_INTL("¡Las emociones de {1} alcanzaron un punto álgido!\n¡Entró en Hiperestado!", self.pbThis))
    end
  end
...
def pbRaiseHappinessAndReduceHeart(pkmn, scene, multiplier, show_fail_message = true)
  if !pkmn.shadowPokemon? || (pkmn.happiness == 255 && pkmn.heart_gauge == 0)
    scene.pbDisplay(_INTL("No tendría ningún efecto.")) if show_fail_message
    return false
  end
  ...
  if pkmn.heart_gauge == old_gauge
    scene.pbDisplay(_INTL("{1} se volvió más amistoso.", pkmn.name))
  elsif pkmn.happiness == old_happiness
    scene.pbDisplay(_INTL("¡{1} te adora!\nLa puerta de su corazón se abrió un poco.", pkmn.name))
    pkmn.check_ready_to_purify
  else
    scene.pbDisplay(_INTL("{1} se volvió más amistoso.\nLa puerta a su corazón se abrió un poco.", pkmn.name))
    pkmn.check_ready_to_purify
  end
  return true
end
...
ItemHandlers::UseOnPokemon.add(:JOYSCENT, proc { |item, qty, pkmn, scene|
  ret = false
  if pkmn.hyper_mode
    scene.pbDisplay(_INTL("{1} recupera su cordura gracias a {2}.", pkmn.name, GameData::Item.get(item).name))
    pkmn.hyper_mode = false
    ret = true
  end
...
ItemHandlers::BattleUseOnPokemon.add(:JOYSCENT, proc { |item, pokemon, battler, choices, scene|
  if pokemon.hyper_mode
    ...
    scene.pbDisplay(_INTL("¡{1} recuperó su cordura gracias a {2}!",
...
class Battle::Move::RemoveAllScreensAndSafeguard < Battle::Move
  ...
  def pbEffectGeneral(user)
    ...
    @battle.pbDisplay(_INTL("¡Ha roto todas las barreras!"))
  end
end
