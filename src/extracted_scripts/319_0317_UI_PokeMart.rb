...
class PokemonMartAdapter
  ...
  def getDisplayName(item)
    ...
    if item_data.is_machine?
      ...
      move_name = GameData::Move.get(machine).name
      if move_name.length > 13
        move_name = move_name[0, 12] + "."
      end
      item_name = _INTL("{1} {2}", item_name.ljust(4), move_name)
    end
    return item_name
  end
...
class PokemonMart_Scene
  ...
  def pbChooseNumber(helptext, item, maximum)
    ...
    using(numwindow = Window_AdvancedTextPokemon.new("")) do   # Showing number of items
      ...
      numwindow.text = _INTL("x{1}<r>{2} Moedas", curnumber, (curnumber * itemprice).to_s_formatted)
...
def pbPokemonMart(stock, speech = nil, cantsell = false)
  ...
  begin
    ...
    cmd = pbMessage(speech || _INTL("¡Bienvenido! ¿En qué te posso ajudar?"), commands, cmdQuit + 1)
    loop do
