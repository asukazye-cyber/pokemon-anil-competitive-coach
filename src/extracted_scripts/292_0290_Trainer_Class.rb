...
class Trainer
  ...
  def party_full?
    return party_count >= max_party_size #Settings::MAX_PARTY_SIZE
  end
  ...
  # form and exclude_form can be integers or arrays of integers.
  def has_species?(species, form = -1, exclude_form = -1, check_pc = false)
    # Check party first
    party_result = pokemon_party.any? { |pkmn| 
      next false unless pkmn&.isSpecies?(species)
      
      # Handle form checking (can be integer or array)
      form_match = if form.is_a?(Array)
                     form.empty? || form.include?(pkmn.form)
                   else
                     form < 0 || pkmn.form == form
                   end
      
      # Handle exclude_form checking (can be integer or array)
      exclude_form_match = if exclude_form.is_a?(Array)
                             exclude_form.empty? || !exclude_form.include?(pkmn.form)
                           else
                             exclude_form < 0 || pkmn.form != exclude_form
                           end
      
      form_match && exclude_form_match
    }
    ...
    if check_pc && $PokemonStorage
      (0...$PokemonStorage.maxBoxes).each do |i|
        $PokemonStorage.maxPokemon(i).times do |j|
          pkmn = $PokemonStorage[i, j]
          next unless pkmn && pkmn.isSpecies?(species)
          ...
          return true if form_match && exclude_form_match
        end
      end
    end
  ...
  def has_species_in_order?(species, form = -1, exclude_form = -1, position)
    # Verificar que la posición sea válida
    return false if position < 0 || position >= pokemon_party.length
    
    pkmn = pokemon_party[position]
    return false unless pkmn&.isSpecies?(species)
