...
def setBattleRule(*args)
  ...
  raise _INTL("Argumento {1} esperaba una variable después pero no la tiene.", r) if r
end
...
module BattleCreationHelperMethods
  ...
  def partner_can_participate?(foe_party)
    ...
    # Force quest/story companion participation
    ...
    if is_story
      if $game_temp.battle_rules["size"]
        return false if $game_temp.battle_rules["size"] == "single" ||
                        $game_temp.battle_rules["size"][/^1v/i]
      end
      return true
    end
  ...
  def set_up_player_trainers(foe_party)
    ...
    party_starts  = [0]
    can_part = partner_can_participate?(foe_party)
    if can_part
  ...
  def prepare_battle(battle)
    ...
    if !battleRules["size"].nil?
      battle.setBattleMode(battleRules["size"])
    end
    ...
    if battleRules["defaultTerrain"].nil?
      if Settings::OVERWORLD_WEATHER_SETS_BATTLE_TERRAIN
        ...
        # Storm NÃO seta Electric Terrain — era bug visual (amarelão)
        when :Fog
