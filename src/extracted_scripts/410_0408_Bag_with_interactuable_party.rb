...
class SpriteWindow_Selectable < SpriteWindow_Base
  ...
  def update
    ...
    if self.active && @item_max > 0 && @index >= 0 && !@ignore_input
      # Lazy global mouse/touch variable initialization
      ...
      if Input.trigger?(Input::MOUSELEFT)
        ...
        if mousepos
          ...
          if mousepos[0] >= self.x + v_x && mousepos[0] < self.x + v_x + self.width &&
            ...
            raw_x = Input.mouse_x
            raw_y = Input.mouse_y
            Mouse.log_click(raw_x, raw_y, mousepos[0], mousepos[1]) rescue nil
            
            is_calib_mode = (Mouse.press_calibration_any?) rescue false
            if is_calib_mode
              pbPlayDecisionSE rescue nil
              rect = clicked_item >= 0 ? self.itemRect(clicked_item) : nil
              target_center_x = rect ? (self.x + v_x + self.startX + rect.x + rect.width / 2) : 0
              target_center_y = rect ? (self.y + v_y + self.startY + rect.y + rect.height / 2) : 0
              offset_x = rect ? (target_center_x - mousepos[0]) : 0
              offset_y = rect ? (target_center_y - mousepos[1]) : 0
              
              log_line = "[#{Time.now.strftime('%Y-%m-%d %H:%M:%S')}] Tipo: MenuCalib | Nome/Pos: #{self.class.name}_Item#{clicked_item} | Raw Mouse: (#{raw_x}, #{raw_y}) | Target Center: (#{target_center_x}, #{target_center_y}) | Offset: (X: #{offset_x}, Y: #{offset_y})\n"
              File.open("mouse_calibration_log.txt", "a") { |f| f.write(log_line) } rescue nil
              File.open("mouse_clicks_log.txt", "a") { |f| f.write(log_line) } rescue nil

              if rect
                point, compiled = Mouse.handle_calibration_click(raw_x, raw_y, target_center_x, target_center_y)
                if point == 0
                  Kernel.pbMessage(_INTL("Ponto 0 registrado! Segure CTRL e clique em outro item para concluir.")) rescue nil
                else
                  if compiled
                    Kernel.pbMessage(_INTL("Calibracao global ATIVADA com sucesso!")) rescue nil
                  else
                    Kernel.pbMessage(_INTL("Ponto 5 registrado, mas a calibracao falhou.")) rescue nil
                  end
                end
              end
            else
              log_line = "[#{Time.now.strftime('%H:%M:%S')}] MENU CLICK - Window: #{self.class.name} | Item: #{clicked_item} | Index: #{@index} | Mouse: (#{mousepos[0]}, #{mousepos[1]}) | cx: #{cx}, cy: #{cy}\n"
              File.open("mouse_clicks_log.txt", "a") { |f| f.write(log_line) } rescue nil
      ...
      if Input.press?(Input::MOUSELEFT) && @global_drag_start_y
        mousepos = Mouse.getMousePos
        if mousepos
          ...
          if @global_has_dragged
            row_diff = (dy / self.rowHeight).to_i
            if row_diff != 0
              new_index = @global_drag_start_index - (row_diff * @column_max)
      ...
      if (Input.release?(Input::MOUSELEFT) rescue false) || !Input.press?(Input::MOUSELEFT)
        @global_drag_start_x = nil
        @global_drag_start_y = nil
        @global_drag_start_index = nil
        @global_has_dragged = false
...
class PokemonBagPartyPanel < Sprite
  ...
  def pokemon=(value)
    if @pokemon.equal?(value)
      stats_changed = false
      if @pokemon.hp != value.hp || @pokemon.totalhp != value.totalhp ||
         @pokemon.status != value.status || @pokemon.level != value.level ||
         @pokemon.pokerusStage != value.pokerusStage || @pokemon.shiny? != value.shiny? ||
         @pokemon.gender != value.gender || @pokemon.item != value.item
        stats_changed = true
      end
      if !stats_changed
...
class PokemonBag_Scene
  ...
  def pbStartScene(bag, party = $player.party, choosing = false, filterproc = nil, resetpocket = true)
    ...
    @party      = party
    @party_already_refreshed_this_scroll = false
    @last_refreshed_pocket = -1
  ...
  text = PTBR_TEXT.t(text) if text.is_a?(String)
  ...
  def pbRefresh
    current_pocket = @sprites["itemlist"].pocket
    if @last_refreshed_pocket != current_pocket
      @last_refreshed_pocket = current_pocket
      @party_already_refreshed_this_scroll = false
      ...
      pocketAcc = current_pocket - 1 # Current pocket
  ...
  def pbRefreshParty
    return if @party_already_refreshed_this_scroll
    @party_already_refreshed_this_scroll = true
    for i in 0...max_party_size
      if @party[i]
  ...
  def pbUpdateAnnotation
    ...
    # OTIMIZAÇÃO: Só recalcula se o item mudou
    @last_annotated_item ||= -1
    return if @last_annotated_item == item
    @last_annotated_item = item
    
    $bag_compatibility_cache ||= {}
    if item && $bag_compatibility_cache.has_key?(item)
      cached = $bag_compatibility_cache[item]
      annotations = cached[0]
      color_annotations = cached[1]
      for i in 0...max_party_size
    ...
    if @bag.last_viewed_pocket == 1 && item #Items Pocket
      ...
      if item
        $bag_compatibility_cache[item] = [annotations, color_annotations]
      end
      ...
      loop do
        ...
        if itemwindow.sorting
          ...
          if Input.trigger?(Input::LEFT)
            ...
            mousepos = Mouse.getMousePos
            if mousepos
              if Input.press?(Input::CTRL) || Input.press?(Input::SHIFT) || Input.press?(Input::SPECIAL)
                rect = itemwindow.itemRect(itemwindow.index)
                viewport = itemwindow.viewport rescue nil
                v_x = (viewport && !viewport.disposed?) ? (viewport.rect.x - viewport.ox) : 0
                v_y = (viewport && !viewport.disposed?) ? (viewport.rect.y - viewport.oy) : 0
                
                target_y = itemwindow.y + v_y + 16 + rect.y + rect.height / 2
                target_x = itemwindow.x + v_x + 16 + rect.x + rect.width / 2
                if itemwindow.class.name == "Window_PokemonBag"
                  target_y += 16
                end
                
                raw_mx = mousepos[0] - $mouse_x_offset
                raw_my = mousepos[1] - $mouse_y_offset
                
                $mouse_x_offset = target_x - raw_mx
                $mouse_y_offset = target_y - raw_my
                
                itm_name = itemwindow.item ? GameData::Item.get(itemwindow.item).name : "CERRAR MOCHILA"
                Mouse.log_calibration("Bolsa", "Index #{itemwindow.index} (#{itm_name})", raw_mx, raw_my, target_x, target_y, $mouse_x_offset, $mouse_y_offset)
                
                Mouse.save_calibration
                pbPlayDecisionSE
                pbMessage(_INTL("Mouse calibrado! Novo Desvio X: {1}, Y: {2}", $mouse_x_offset, $mouse_y_offset))
                next
              end
          ...
          if Input.press?(Input::MOUSELEFT) && @drag_start_y
            mousepos = Mouse.getMousePos
            if mousepos
              ...
              if @has_dragged
                ...
                v_x = (viewport && !viewport.disposed?) ? (viewport.rect.x - viewport.ox) : 0
                v_y = (viewport && !viewport.disposed?) ? (viewport.rect.y - viewport.oy) : 0
                y_check = itemwindow.y
                h_check = itemwindow.height
                if itemwindow.class.name == "Window_PokemonBag"
                  y_check += 16
                  h_check -= 16
                end
                if mousepos[0] >= itemwindow.x + v_x && mousepos[0] < itemwindow.x + v_x + itemwindow.width &&
                   mousepos[1] >= y_check + v_y && mousepos[1] < y_check + v_y + h_check
                  threshold = 16
                  if dy.abs >= threshold
                    row_diff = (dy / threshold).to_i
                    new_index = @drag_start_index - row_diff
                    new_index = [0, [new_index, itemwindow.itemCount - 1].min].max
                    if itemwindow.index != new_index
                      itemwindow.index = new_index
                      pbRefresh
                    end
                    @drag_start_y = mousepos[1]
                    @drag_start_x = mousepos[0]
          ...
          if Input.release?(Input::MOUSELEFT)
            if !@has_dragged
              mousepos = Mouse.getMousePos
              if mousepos
                # 1. Check if click is on pocket icons
                if mousepos[0] >= 372 && mousepos[0] < 504 && mousepos[1] >= 0 && mousepos[1] < 54
                  pocketX = []
                  incrementX = 0
                  @bag.pockets.length.times do |p_idx|
                    pocketX.push(incrementX)
                    incrementX += 2 if p_idx.odd?
                  end
                  clicked_pocket = -1
                  (1...@bag.pockets.length).each do |i|
                    pocketValue = i - 1
                    px = 372 + (i - 1) * 14 + pocketX[pocketValue]
                    py = (i % 2) * 26
                    if mousepos[0] >= px && mousepos[0] < px + 28 && mousepos[1] >= py && mousepos[1] < py + 28
                      if !@choosing || (@filterlist ? @filterlist[i].length > 0 : @bag.pockets[i].length > 0)
                        clicked_pocket = i
                        break
                      end
                    end
                  end
                  if clicked_pocket >= 1 && clicked_pocket != itemwindow.pocket
                    itemwindow.pocket = clicked_pocket
                    @bag.last_viewed_pocket = itemwindow.pocket
                    thispocket = @bag.pockets[itemwindow.pocket]
                    pbRefresh
                    pbSEPlay("Voltorb Flip mark")
                ...
                # 2. Check if click is on the item list
                else
                  viewport = itemwindow.viewport rescue nil
                  v_x = (viewport && !viewport.disposed?) ? (viewport.rect.x - viewport.ox) : 0
                  v_y = (viewport && !viewport.disposed?) ? (viewport.rect.y - viewport.oy) : 0
                  border_x = itemwindow.respond_to?(:borderX) ? itemwindow.borderX : 32
                  border_y = itemwindow.respond_to?(:borderY) ? itemwindow.borderY : 32
                  cx = mousepos[0] - (itemwindow.x + v_x + border_x / 2)
                  cy = mousepos[1] - (itemwindow.y + v_y + border_y / 2)
                  if itemwindow.class.name == "Window_PokemonBag"
                    cy -= 16
                  end
                  y_check = itemwindow.y
                  ...
                  if mousepos[0] >= itemwindow.x + v_x && mousepos[0] < itemwindow.x + v_x + itemwindow.width &&
                     mousepos[1] >= y_check + v_y && mousepos[1] < y_check + v_y + h_check
                    clicked_item = -1
                    top = itemwindow.top_item
                    limit = [top + itemwindow.page_item_max, itemwindow.itemCount].min
                    (top...limit).each do |i|
                      rect = itemwindow.itemRect(i)
                      # Restrict to actual item row click width to prevent misclicks on blank margins
                      if cx >= rect.x && cx < rect.x + rect.width && cy >= rect.y && cy < rect.y + rect.height
                        clicked_item = i
                        break
                      end
                    ...
                    if clicked_item >= 0
                      if itemwindow.index == clicked_item
                        (itemwindow.item) ? pbPlayDecisionSE : pbPlayCloseMenuSE
                        return itemwindow.item
                      else
                        itemwindow.index = clicked_item
                        pbPlayCursorSE
                        pbRefresh
                      end
                    end
    ...
    loop do
      ...
      if Input.trigger?(Input::MOUSELEFT)
        ...
        if mousepos
          ...
          if Mouse.press_calibration_any? || Input.press?(Input::CTRL) || Input.press?(Input::SHIFT) || Input.press?(Input::SPECIAL)
            ...
            if sprite && !sprite.disposed?
              ...
              w = bg && !bg.disposed? && bg.bitmap ? bg.bitmap.width : 110
              h = bg && !bg.disposed? && bg.bitmap ? bg.bitmap.height : 100
              ...
              Mouse.log_calibration("Bolsa_Party", "Slot #{@activecmd} (#{pkmn_name})", raw_mx, raw_my, target_x, target_y, $mouse_x_offset, $mouse_y_offset)
          ...
          max_party_size.times do |i|
            ...
            if Mouse.is_over_sprite?(sprite, mousepos[0], mousepos[1]) && mousepos[0] < 204
              if i < @party.length && @party[i]
                clicked_idx = i
                break
              end
            end
      ...
      if Input.trigger?(Input::USE) || clicked_by_mouse
...
class PokemonBagScreen
  def initialize(scene, bag, send_peer = nil)
    @bag       = bag
    @scene     = scene
    @send_peer = send_peer
  end

  def can_send_item_to_peer?(item)
    return false if !@send_peer
    return false if !defined?(AnilLanRework::ItemSync)
    return false if !AnilLanRework::ItemSync.peer_available?(@send_peer)
    itm = GameData::Item.try_get(item)
    return false if !itm || itm.is_important?
    return @bag.quantity(item) > 0
  rescue
    false
  end

  def pbSendItemToPeer(item)
    return false unless can_send_item_to_peer?(item)
    itm = GameData::Item.get(item)
    qty = @bag.quantity(item)
    if qty > 1
      helptext = _INTL("Que quantidade de {1} enviar?", itm.portion_name_plural)
      qty = @scene.pbChooseNumber(helptext, qty)
    end
    return false if qty <= 0

    itemname = (qty > 1) ? itm.portion_name_plural : itm.portion_name
    peer_name = AnilLanRework::ItemSync.peer_display_name(@send_peer)
    return false if !pbConfirm(_INTL("Enviar {1} {2} para {3}?", qty, itemname, peer_name))

    success, message = AnilLanRework::ItemSync.send_item(@send_peer, item, qty)
    pbDisplay(message) if message && !message.to_s.empty?
    @scene.pbRefresh
    return success
  end
  ...
  def pbStartScreen
    ...
    loop do
      ...
      commands[cmdSend = commands.length]       = _INTL("Enviar") if can_send_item_to_peer?(item)
      ...
      elsif cmdSend >= 0 && command == cmdSend   # Send item to partner
        pbSendItemToPeer(item)
