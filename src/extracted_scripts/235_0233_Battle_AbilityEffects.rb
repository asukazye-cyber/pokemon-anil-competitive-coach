...
module Battle::AbilityEffects
  ...
  def self.triggerOnEndOfUsingMove(ability, user, targets, move, battle, switched_battlers)
    OnEndOfUsingMove.trigger(ability, user, targets, move, battle, switched_battlers)
  end
...
Battle::AbilityEffects::SpeedCalc.add(:ACOMETIDA,
  proc { |ability, battler, mult|
    next mult * 1.5 if battler.turnCount == 0
  }
)
...
Battle::AbilityEffects::SpeedCalc.add(:PODERGELIDO,
  proc { |ability, battler, mult|
    next mult * 2 if [:Hail].include?(battler.effectiveWeather)
  }
)
...
# Battle::AbilityEffects::OnHPDroppedBelowHalf.copy(:WIMPOUT, :EMERGENCYEXIT)
...
Battle::AbilityEffects::ModifyMoveBaseType.add(:LIQUIDVOICE,
  proc { |ability, user, move, type|
    next if !GameData::Type.exists?(:WATER) || !move.soundMove?
    move.powerBoost = true
    next :WATER 
  }
)
...
Battle::AbilityEffects::ModifyMoveBaseType.add(:PIELHERBACEA,
  proc { |ability, user, move, type|
...
Battle::AbilityEffects::AccuracyCalcFromUser.add(:HUSTLE,
  proc { |ability, mods, user, target, move, type|
    mods[:accuracy_multiplier] *= 0.8 if move.physicalMove? || move.specialMove?
  }
)
...
Battle::AbilityEffects::DamageCalcFromUser.add(:LIQUIDVOICE,
  proc { |ability, user, target, move, mults, power, type|
    mults[:power_multiplier] *= 1.2 if move.powerBoost && move.soundMove?
  }
)

# Battle::AbilityEffects::DamageCalcFromUser.add(:TINTINEO,
#   proc { |ability, user, target, move, mults, power, type|
#     mults[:power_multiplier] *= 1.2 if move.powerBoost
#   }
# )
...
Battle::AbilityEffects::DamageCalcFromUser.copy(:AERILATE, :GALVANIZE, :LIQUIDVOICE, :NORMALIZE, :PIXILATE, :REFRIGERATE, :PIELMALDITA, :PIELELECTRICA, :PIELHERBACEA, :TINTINEO)
...
Battle::AbilityEffects::DamageCalcFromUser.add(:PODERSABIO,
  proc { |ability, user, target, move, mults, power, type|
...
Battle::AbilityEffects::DamageCalcFromUser.add(:HUSTLE,
  proc { |ability, user, target, move, mults, power, type|
    mults[:attack_multiplier] *= 1.5 if move.physicalMove? || move.specialMove?
  }
)
...
Battle::AbilityEffects::DamageCalcFromUser.add(:CAMORRISTA,
  proc { |ability, user, target, move, mults, power, type|
...
Battle::AbilityEffects::DamageCalcFromUser.add(:SOBRECARGA,
  proc { |ability, user, target, move, mults, power, type|
...
Battle::AbilityEffects::DamageCalcFromUser.add(:ACOMETIDA,
  proc { |ability, user, target, move, mults, power, type|
    mults[:attack_multiplier] *= 1.3 if user.turnCount == 0
  }
)
...
Battle::AbilityEffects::DamageCalcFromUser.add(:COLEOPTERO,
  proc { |ability, user, target, move, mults, power, type|
    mults[:attack_multiplier] *= 1.5 if type == :BUG
  }
)

Battle::AbilityEffects::DamageCalcFromUser.add(:INFLAMABLE,
  proc { |ability, user, target, move, mults, power, type|
...
Battle::AbilityEffects::DamageCalcFromUser.add(:SILVANO,
  proc { |ability, user, target, move, mults, power, type|
    mults[:attack_multiplier] *= 1.3 if user.battle.field.terrain == :Grassy 
  }
)

Battle::AbilityEffects::DamageCalcFromUser.add(:PODERGELIDO,
  proc { |ability, user, target, move, mults, power, type|
    mults[:attack_multiplier] *= 1.5 if type == :ICE && [:HAIL].include?(user.effectiveWeather)
  }
)

Battle::AbilityEffects::DamageCalcFromUser.add(:FLORACION,
  proc { |ability, user, target, move, mults, power, type|
    mults[:attack_multiplier] *= 1.5 if type == :GRASS
  }
)

Battle::AbilityEffects::DamageCalcFromUser.add(:REALEZA,
  proc { |ability, user, target, move, mults, power, type|
    # Increase power only if the move is not already from the user stab
    mults[:attack_multiplier] *= 1.5 unless user.types.include?(type)
  }
)
...
Battle::AbilityEffects::OnEndOfUsingMove.add(:EMERGENCYEXIT,
  proc { |ability, user, targets, move, battle, switched_battlers|
    next if move.calcType != :BUG
    # In wild battles
    if battle.wildBattle? && user.opposes?
      # next false if battle.pbSideBattlerCount(user.index) <= 1
      next false if !battle.pbCanRun?(user.index)
      battle.pbShowAbilitySplash(user, true)
      battle.pbHideAbilitySplash(user)
      pbSEPlay("Battle flee")
      battle.pbDisplay(_INTL("¡{1} ha huido!", user.pbThis))
      battle.decision = 3   # Escaped
      next true
    end

     # In trainer battles
     next false if battle.pbAllFainted?(user.idxOpposingSide)
     next false if !battle.pbCanSwitchOut?(user.index)   # Battler can't switch out
     next false if !battle.pbCanChooseNonActive?(user.index)   # No Pokémon can switch in
     battle.pbShowAbilitySplash(user, true)
     battle.pbHideAbilitySplash(user)
     if !Battle::Scene::USE_ABILITY_SPLASH
       battle.pbDisplay(_INTL("¡La habilidad {2} de {1} se ha activado!", user.pbThis(true), user.abilityName))
     end
     battle.pbDisplay(_INTL("¡{1} regresó con {2}!", user.pbThis, battle.pbGetOwnerName(user.index)))
     battle.pbPursuit(user.index)
     next false if user.fainted?
     if battle.endOfRound   # Just switch out
       battle.scene.pbRecall(user.index) if !user.fainted?
       user.pbAbilitiesOnSwitchOut   # Inc. primordial weather check
       next true
     end
     ...
     battle.moldBreaker = false #if newPkmn && targets.any? { |b| b.index == newPkmn.index }
     # Mark that this battler switched mid-move to prevent Life Orb recoil on the replacement
...
Battle::AbilityEffects::OnDealingHit.add(:BURNTOUCH,
  proc { |ability, user, target, move, battle|
    next if !move.contactMove?
    next if battle.pbRandom(100) >= 30
    next if target.hasActiveItem?(:COVERTCLOAK)
    ...
    if target.hasActiveAbility?(:SHIELDDUST) && !battle.moldBreaker
      ...
      msg = nil
      if !Battle::Scene::USE_ABILITY_SPLASH
        msg = _INTL("¡La habilidad {2} de {1} quemó a {3}!", user.pbThis(true), user.abilityName, target.pbThis(true))
      end
...
Battle::AbilityEffects::OnSwitchIn.add(:ACOMETIDA,
  proc { |ability, battler, battle, switch_in|
    battle.pbShowAbilitySplash(battler)
    if Battle::Scene::USE_ABILITY_SPLASH
      battle.pbDisplay(_INTL("¡{1} entra a combatir con furia desmedida!", battler.pbThis))
    else
      battle.pbDisplay(_INTL("¡{1} entra a combatir con furia desmedida por su habilidad {2}!",
         battler.pbThis, battler.abilityName))
    end
    battle.pbHideAbilitySplash(battler)
  }
...
Battle::AbilityEffects::OnSwitchIn.add(:TINTINEO,
  proc { |ability, battler, battle, switch_in|
    battle.pbShowAbilitySplash(battler)
    if !Battle::Scene::USE_ABILITY_SPLASH
      battle.pbDisplay(_INTL("¡{1} tintinea como una campana por su habilidad {2}!", battler.pbThis, battler.abilityName))
    else
      battle.pbDisplay(_INTL("¡{1} tintinea como una campana!", battler.pbThis))
    end
    battle.pbHideAbilitySplash(battler)
    
    battle.allSameSideBattlers(battler).each do |b|
      b.pbCureStatus(true) if b.status != :NONE
    end

    battle.pbParty(battler.index).each_with_index do |pkmn, i|
      next if !pkmn || !pkmn.able? || pkmn.status == :NONE
      next if battle.pbFindBattler(i, battler)   # Skip Pokémon in battle
      oldStatus = pkmn.status
      pkmn.status      = :NONE
      pkmn.statusCount = 0
      case oldStatus
      when :SLEEP
        battle.pbDisplay(_INTL("{1} se despertó.", pkmn.name))
      when :POISON
        battle.pbDisplay(_INTL("{1} fue curado del envenenamiento.", pkmn.name))
      when :BURN
        battle.pbDisplay(_INTL("La quemadura de {1} fue curada.", pkmn.name))
      when :PARALYSIS
        battle.pbDisplay(_INTL("{1} fue curado de la parálisis.", pkmn.name))
      when :FROZEN
        battle.pbDisplay(_INTL("{1} se descongeló.", pkmn.name))
      when :FROSTBITE
        battle.pbDisplay(_INTL("{1} se descongeló.", pkmn.name))
      end
    end
...
Battle::AbilityEffects::ModifyTypeEffectiveness.add(:TERASHELL,
  proc { |ability, user, target, move, battle, effectiveness|
    next if !move.damagingMove?
    next if move.pbIsChargingTurn?(user) if defined?(move.pbIsChargingTurn?)
