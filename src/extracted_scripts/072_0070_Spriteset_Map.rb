...
class Spriteset_Map
  ...
  def initialize(map = nil)
    ...
    renderer = ($scene.map_renderer if $scene.is_a?(Scene_Map) rescue nil)
    if renderer
      renderer.add_tileset(@map.tileset_name)
      @map.autotile_names.each { |filename| renderer.add_autotile(filename) }
      renderer.add_extra_autotiles(@map.tileset_id)
    end
  ...
  def dispose
    renderer = ($scene.map_renderer if $scene.is_a?(Scene_Map) rescue nil)
    if renderer
      renderer.remove_tileset(@map.tileset_name)
      @map.autotile_names.each { |filename| renderer.remove_autotile(filename) }
      renderer.remove_extra_autotiles(@map.tileset_id)
    end
    @panorama.dispose rescue nil
    @fog.dispose rescue nil
    @character_sprites.each { |sprite| sprite.dispose rescue nil } if @character_sprites
    @panorama = nil
    @fog = nil
    @character_sprites.clear if @character_sprites
  end
