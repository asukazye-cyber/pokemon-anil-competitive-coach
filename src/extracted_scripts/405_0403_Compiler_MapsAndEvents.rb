module Compiler
  SCRIPT_REPLACEMENTS = [
    ...
    ["EncounterTypes:",              ""],
    [":TALADOR",                     ":CUTITEM"],
    [":ILUMINADOR",                  ":FLASHITEM"],
    [":SURFMONTURA",                 ":SURFITEM"],
    [":Escalada",                    ":ROCKCLIMBITEM"],
    ...
    ["Graphics.frame_rate=40",       "$GameSpeed = 0"],
    ["Graphics.frame_rate = 40",     "$GameSpeed = 0"],
    [":TABLANEUTRA",                 ":BLANKPLATE"]
  ...
  class MapData
    ...
    def getTilesetPassages(map, mapID)
      begin
        ...
        raise "Los datos del tileset para el número de tileset #{map.tileset_id} utilizado en el mapa #{mapID} no se encontraron. " +
            "Es probable que el tileset haya sido eliminado, pero uno o más mapas todavía lo están utilizando."
      end
    end
    ...
    def isPassable?(mapID, x, y)
      ...
      [2, 1, 0].each do |i|
        ...
        if !passage
          raise "La ficha utilizada en el mapa #{mapID} en las coordenadas (#{x}, #{y}) en la capa #{i + 1} no existe en el conjunto de tiles. " +
                "Debería eliminarse para evitar errores."
        end
    ...
    def isCounterTile?(mapID, x, y)
      ...
      [2, 1, 0].each do |i|
        ...
        passage = passages[tile_id]
        if !passage
          raise "El tile utilizada en el mapa #{mapID} en las coordenadas (#{x}, #{y}) en la capa #{i + 1} no existe en el conjunto de tiles. " +
                "Debería eliminarse para evitar errores."
        end
  ...
  def convert_to_trainer_event(event, trainerChecker)
    ...
    commands.each do |command|
      if command[/^Battle\:\s*([\s\S]+)$/i]
        ...
        doublebattle = true if value.upcase == "TRUE" || value.upcase == "Sí"
        push_comment(firstpage.list, command) if rewriteComments
      elsif command[/^Continue\:\s*([\s\S]+)$/i]
        value = $~[1].gsub(/^\s+/, "").gsub(/\s+$/, "")
        continue = true if value.upcase == "TRUE" || value.upcase == "Sí"
        push_comment(firstpage.list, command) if rewriteComments
    ...
    if battles.length > 1   # Has rematches
      ...
      push_choices(firstpage.list, ["Sí", "No"], 2, 2)
      push_choice(firstpage.list, 0, "Sí", 3)
      if common_event > 0
        if battleid > 0
    ...
    if battles.length > 1
      ...
      push_choices(lastpage.list, ["Sí", "No"], 2, 1)
      push_choice(lastpage.list, 0, "Sí", 2)
      if common_event > 0
        if battleid > 0
  ...
  # Checks if the event's name is "Objeto:POTION" or "ObjetoOculto:POTION". If so,
  # rewrites the whole event into one now named "Objeto"/"ObjetoOculto" which gives
  ...
  def convert_to_item_event(event)
    ...
    itemName = ""
    hidden = false
    if name[/^objetooculto\:\s*(\w+)\s*$/i]
      itemName = $1
      return nil if !GameData::Item.exists?(itemName)
      ret.name = "ObjetoOculto"
      hidden = true
    elsif name[/^objeto\:\s*(\w+)\s*$/i]
      itemName = $1
      return nil if !GameData::Item.exists?(itemName)
      ret.name = "Objeto"
    else
...
#       mapData.isPassable?(mapID,thisEvent.x,thisEvent.y+1) &&
  ...
  def fix_event_name(event)
    ...
    when "tree"
      event.name = "ArbolCorte"
    when "rock"
      event.name = "RocaRompible"
    when "boulder"
      event.name = "PiedraFuerza"
    #when "antorcha glowsize(150)"
    #  event.name = "antorcha glowsize(80)"
    else
      return false
    end
    return true
  ...
  # def event_has?(event,code,script)
  #   ret = false
  ...
  #       case cmd.code
  #       when code
  #         ret = true if params.include?(script)
  #       end
  #     end
  #   end
  #   return ret
          ...
          if params[0][/SellItem\s*\(\s*(\w+)\s*\,\s*(\d+)\s*\)/]
            ...
            if GameData::Item.exists?(itemname)
              ...
              if cost == 0
                ...
                push_text(newEvents, _INTL("\\G¡Aquí tienes!"), oldIndent + 2)
                push_script(newEvents, "pbReceiveItem(:#{itemname})", oldIndent + 2)
                push_else(newEvents, oldIndent + 2)
                push_text(newEvents, _INTL("\\GNo te queda espacio en la Mochila."), oldIndent + 2)
                push_branch_end(newEvents, oldIndent + 2)
                push_else(newEvents, oldIndent + 1)
                push_text(newEvents, _INTL("\\GNo tienes suficiente dinero."), oldIndent + 1)
              end
...
#            end
  ...
  def compile_trainer_events(_mustcompile)
    ...
    change_record = []
    Console.echo_li(_INTL("Procesando {1} mapas...", mapData.mapinfos.keys.length))
    idx = 0
    mapData.mapinfos.keys.sort.each do |id|
      ...
      if changed
        ...
        change_record.push(_INTL("El mapa {1}: '{2}' se ha modificado y guardado.", id, mapData.mapinfos[id].name))
      end
    end
