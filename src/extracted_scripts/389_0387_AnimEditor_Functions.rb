module BattleAnimationEditor
  ...
  def pbChangeMaximum(canvas)
    ...
    cancelbutton = sliderwin2.addButton(_INTL("Cancelar"))
  ...
  def pbAnimName(animation, cmdwin)
    ...
    cancelbutton = window.addButton(_INTL("Cancelar"))
    window.opacity = 224
    loop do
      Graphics.update
      Input.update
  ...
  def pbAnimList(animations, canvas, animwin)
    ...
    maxsizewindow.addSlider(_INTL("Animaciones Totales:"), 1, 2000, animations.length)
    maxsizewindow.addButton(_INTL("Cambiar tamaño de Lista de Anim."))
  ...
  def pbChooseNum(cel)
    ...
    sliderwin2.z = 99999
    sliderwin2.addLabel(_INTL("Número Antiguo: {1}", cel))
    sliderwin2.addSlider(_INTL("Número Nuevo:"), 2, PBAnimation::MAX_SPRITES, cel)
    okbutton = sliderwin2.addButton(_INTL("OK"))
    cancelbutton = sliderwin2.addButton(_INTL("Cancelar"))
    loop do
      Graphics.update
  ...
  def pbSetTone(cel, previewsprite)
    ...
    sliderwin2.z = 99999
    sliderwin2.addSlider(_INTL("Offset Rojo:"), -255, 255, cel[AnimFrame::TONERED])
    sliderwin2.addSlider(_INTL("Offset Verde:"), -255, 255, cel[AnimFrame::TONEGREEN])
    sliderwin2.addSlider(_INTL("Offset Azul:"), -255, 255, cel[AnimFrame::TONEBLUE])
    sliderwin2.addSlider(_INTL("Tono Gris:"), 0, 255, cel[AnimFrame::TONEGRAY])
    okbutton = sliderwin2.addButton(_INTL("OK"))
    cancelbutton = sliderwin2.addButton(_INTL("Cancelar"))
    loop do
  ...
  def pbCellProperties(canvas)
    ...
    set0 = sliderwin2.addSlider(_INTL("Patrón:"), -2, 1000, cel[AnimFrame::PATTERN])
    ...
    set5 = sliderwin2.addSlider(_INTL("Ángulo:"), 0, 359, cel[AnimFrame::ANGLE])
    set6 = sliderwin2.addSlider(_INTL("Opacidad:"), 0, 255, cel[AnimFrame::OPACITY])
    ...
    flashbutton = sliderwin2.addButton(_INTL("Establecer Color de Fusión"))
    tonebutton = sliderwin2.addButton(_INTL("Establecer Tono de Color"))
    okbutton = sliderwin2.addButton(_INTL("OK"))
    cancelbutton = sliderwin2.addButton(_INTL("Cancelar"))
  ...
  def pbTimingList(canvas)
    ...
    commands[cmdNewSound = commands.length] = _INTL("Añadir: Reproducir sonido...")
    commands[cmdNewBG = commands.length] = _INTL("Añadir: Definir gráfico de fondo...")
    commands[cmdEditBG = commands.length] = _INTL("Añadir: Editar color/localización del fondo...")
    commands[cmdNewFO = commands.length] = _INTL("Añadir: Establecer gráfico en primer plano...")
    commands[cmdEditFO = commands.length] = _INTL("Añadir: Editar color/localiz. de gráfico en primer plano...")
    ...
    framewindow.addButton(_INTL("Definir Frame"))
    framewindow.addButton(_INTL("Eliminar Tiempo"))
  ...
  def pbSelectSE(canvas, audio)
    ...
    displayname = (filename != "") ? filename : _INTL("<grito del usuario>")
    animfiles = []
    ret = false
  ...
  #    animfiles.concat(Dir.glob("*.mp3"))
    ...
    maxsizewindow.addLabel(_INTL("Archivo: \"{1}\"", displayname))
    maxsizewindow.addSlider(_INTL("Volumen:"), 0, 100, audio.volume)
    maxsizewindow.addSlider(_INTL("Tono:"), 20, 250, audio.pitch)
    maxsizewindow.addButton(_INTL("Reproducir Sonido"))
    maxsizewindow.addButton(_INTL("Parar Sonido"))
    maxsizewindow.addButton(_INTL("OK"))
    maxsizewindow.addButton(_INTL("Cancelar"))
  ...
  def pbSelectBG(canvas, timing)
    ...
    cmdErase = -1
    animfiles = []
    animfiles[cmdErase = animfiles.length] = _INTL("[Eliminar gráfico del fondo]")
    ret = false
  ...
  #    animfiles.concat(Dir.glob("*.bmp"))
    ...
    maxsizewindow.addLabel(_INTL("Archivo: \"{1}\"", filename))
    maxsizewindow.addSlider(_INTL("X:"), -500, 500, timing.bgX || 0)
    maxsizewindow.addSlider(_INTL("Y:"), -500, 500, timing.bgY || 0)
    maxsizewindow.addSlider(_INTL("Opacidad:"), 0, 255, timing.opacity || 0)
    maxsizewindow.addSlider(_INTL("Rojo:"), 0, 255, timing.colorRed || 0)
    maxsizewindow.addSlider(_INTL("Verde:"), 0, 255, timing.colorGreen || 0)
    maxsizewindow.addSlider(_INTL("Azul:"), 0, 255, timing.colorBlue || 0)
    maxsizewindow.addSlider(_INTL("Alfa:"), 0, 255, timing.colorAlpha || 0)
    maxsizewindow.addButton(_INTL("OK"))
    maxsizewindow.addButton(_INTL("Cancelar"))
    maxsizewindow.opacity = 200
    maxsizewindow.viewport = canvas.viewport
  ...
  def pbEditBG(canvas, timing)
    ...
    maxsizewindow.addOptionalSlider(_INTL("Rojo:"), 0, 255, timing.colorRed || 0)
    maxsizewindow.addOptionalSlider(_INTL("Verde:"), 0, 255, timing.colorGreen || 0)
    maxsizewindow.addOptionalSlider(_INTL("Azul:"), 0, 255, timing.colorBlue || 0)
    maxsizewindow.addOptionalSlider(_INTL("Alfa:"), 0, 255, timing.colorAlpha || 0)
    maxsizewindow.addButton(_INTL("OK"))
    maxsizewindow.addButton(_INTL("Cancelar"))
  ...
  def pbCopyFrames(canvas)
    ...
    sliderwin2.viewport = canvas.viewport
    sliderwin2.addSlider(_INTL("Primer Frame:"), 1, canvas.animation.length, 1)
    sliderwin2.addSlider(_INTL("Último Frame:"), 1, canvas.animation.length, canvas.animation.length)
    sliderwin2.addSlider(_INTL("Copiar a:"), 1, canvas.animation.length, canvas.currentframe + 1)
    okbutton = sliderwin2.addButton(_INTL("OK"))
    cancelbutton = sliderwin2.addButton(_INTL("Cancelar"))
    sliderwin2.opacity = 200
    loop do
  ...
  def pbTweening(canvas)
    ...
    sliderwin2.viewport = canvas.viewport
    sliderwin2.opacity = 200
    s1set0 = sliderwin2.addSlider(_INTL("Frame Inicial:"), 1, canvas.animation.length, 1)
    s1set1 = sliderwin2.addSlider(_INTL("Frame Final:"), 1, canvas.animation.length, canvas.animation.length)
    s1set2 = sliderwin2.addSlider(_INTL("Primera Celda:"), 0, PBAnimation::MAX_SPRITES - 1, 0)
    s1set3 = sliderwin2.addSlider(_INTL("Última Celda:"), 0, PBAnimation::MAX_SPRITES - 1, PBAnimation::MAX_SPRITES - 1)
    set0 = sliderwin2.addCheckbox(_INTL("Patrón"))
    set1 = sliderwin2.addCheckbox(_INTL("Posición/Zoom/Ángulo"))
    set2 = sliderwin2.addCheckbox(_INTL("Opacidad/Fusión"))
    okbutton = sliderwin2.addButton(_INTL("OK"))
    cancelbutton = sliderwin2.addButton(_INTL("Cancelar"))
    loop do
      Graphics.update
  ...
  def pbCellBatch(canvas)
    ...
    s1set0 = sliderwin1.addSlider(_INTL("Primer Frame:"), 1, canvas.animation.length, 1)
    s1set1 = sliderwin1.addSlider(_INTL("Último Frame:"), 1, canvas.animation.length, canvas.animation.length)
    s1set2 = sliderwin1.addSlider(_INTL("Primera Celda:"), 0, PBAnimation::MAX_SPRITES - 1, 0)
    s1set3 = sliderwin1.addSlider(_INTL("Última Celda:"), 0, PBAnimation::MAX_SPRITES - 1, PBAnimation::MAX_SPRITES - 1)
    sliderwin2 = ControlWindow.new(300, 0, 340, 32 * 14)
    sliderwin2.viewport = canvas.viewport
    sliderwin2.opacity = 200
    set0 = sliderwin2.addOptionalSlider(_INTL("Patrón:"), -2, 1000, 0)
    ...
    set5 = sliderwin2.addOptionalSlider(_INTL("Ángulo:"), 0, 359, 0)
    set6 = sliderwin2.addOptionalSlider(_INTL("Opacidad:"), 0, 255, 255)
    set7 = sliderwin2.addOptionalSlider(_INTL("Fusión:"), 0, 2, 0)
  ...
  def pbEntireSlide(canvas)
    ...
    sliderwin2.addSlider(_INTL("Primer Frame:"), 1, canvas.animation.length, 1)
    sliderwin2.addSlider(_INTL("Último Frame:"), 1, canvas.animation.length, canvas.animation.length)
    sliderwin2.addSlider(_INTL("Movimiento Eje-X"), -500, 500, 0)
    sliderwin2.addSlider(_INTL("Movimiento Eje-Y"), -500, 500, 0)
    okbutton = sliderwin2.addButton(_INTL("OK"))
    cancelbutton = sliderwin2.addButton(_INTL("Cancelar"))
    sliderwin2.opacity = 200
    loop do
  ...
  def pbAnimEditorHelpWindow
    helptext = "" +
               "Para añadir una celda a la escena, haz clic en el lienzo. La celda seleccionada tendrá un " +
               "borde negro. Después de seleccionar una celda, puedes modificar sus propiedades usando el teclado:\n" +
               "E, R - Rotar a la izquierda/derecha.\nP - Abrir pantalla de propiedades.\nTeclas de flecha - Mover " +
               "la celda 8 píxeles (mantén presionado ALT para 2 píxeles).\n+/- : Acercar/alejar.\nL - Bloquear una " +
               "celda. Bloquear una celda evita que se mueva o elimine.\nDEL - Elimina la celda.\nTambién presiona TAB " +
               "para cambiar la celda seleccionada."
  ...
  def animationEditorMain(animation)
    ...
    sidewin.addButton(_INTL("SE y BG..."))
    sidewin.addButton(_INTL("Foco de Celda..."))
    sidewin.addSpace
    sidewin.addButton(_INTL("pegar Último"))
    sidewin.addButton(_INTL("Copiar Frames..."))
    sidewin.addButton(_INTL("Eliminar Frames..."))
    sidewin.addButton(_INTL("Interpolación..."))
    sidewin.addButton(_INTL("Lote de Celdas..."))
    sidewin.addButton(_INTL("Toda la Diapositiva..."))
    sidewin.addSpace
    sidewin.addButton(_INTL("Reprod. Anim"))
    sidewin.addButton(_INTL("Reprod. Anim. Rival"))
    sidewin.addButton(_INTL("Import. Anim..."))
    sidewin.addButton(_INTL("Export. Anim..."))
    sidewin.addButton(_INTL("Ayuda"))
    ...
    sliderwin.addButton(_INTL("Establecer Hoja de Anim."))
    sliderwin.addButton(_INTL("Lista de Animaciones"))
    ...
    loop do
      ...
      if Input.trigger?(Input::BACK)
        ...
        if pbConfirmMessage(_INTL("¿Salir del editor?"))
      ...
      if Input.triggerex?(:F5)
        ...
        commands = [
          ...
          _INTL("Pegar Frame"),
          _INTL("Vaciar Frame"),
          _INTL("Insertar Frame"),
          _INTL("Eliminar Frame")
        ]
