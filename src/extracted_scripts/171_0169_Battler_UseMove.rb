class Battle::Battler
  ...
  def pbEndTurn(_choice)
    ...
    if !@effects[PBEffects::ChoiceBand] &&
       ...
       hasActiveAbility?(:GORILLATACTICS) || hasActiveAbility?(:PODERSABIO))
  ...
  def pbUseMove(choice, specialUsage = false)
    ...
    if !@battle.futureSight
      if usingMultiTurnAttack? && !@currentMove.nil?
        ...
        specialUsage = true
      elsif usingMultiTurnAttack?
        # ⚠️ ESTADO IMPOSSIVEL: golpe multi-turno a decorrer sem golpe.
        #
        # O pbCancelMoves limpa Rollout/Outrage/Uproar/Bide e poe @currentMove a
        # nil, mas so quando o HyperBeam esta a zero. O TwoTurnAttack e limpo na
        # mesma linha acima — se algo o voltar a definir depois (uma restauracao
        # de estado do coop, um efeito de outro plugin), fica-se com o
        # usingMultiTurnAttack? a dizer SIM e o @currentMove a nil.
        #
        # Na linha de baixo isso virava Pokemon::Move.new(nil) e o jogo caia com
        #   ArgumentError: se esperaba [Symbol, GameData::Move, String], NilClass
        # a meio do turno, no pbProcessTurn — a batalha inteira ia abaixo.
        #
        # Aqui limpa-se o estado orfao e deixa-se o battler usar o golpe que
        # escolheu. Perde-se a continuacao do golpe de dois turnos (que ja estava
        # perdida, porque nao ha golpe nenhum guardado) em vez de perder a
        # batalha.
        @effects[PBEffects::TwoTurnAttack] = nil
        ...
        @effects[PBEffects::Uproar]        = 0
        @effects[PBEffects::Bide]          = 0
        PBDebug.log("[Estado corrigido] #{pbThis} tinha golpe multi-turno sem @currentMove; efeitos limpos.") rescue nil
    ...
    @battle.pbPriority(true).each do |b|
      ...
      if Battle::AbilityEffects.triggerMoveBlocking(b.ability, b, user, targets, move, @battle)
        @battle.pbDisplayBrief(_INTL("¡{1} ha usado {2}!", user.pbThis, move.name))
    ...
    if user.status == :FROZEN && move.thawsUser?
      ...
      @battle.pbDisplay(_INTL("¡{1} derritió el hielo!", user.pbThis))
    end
    # Powder
    ...
    if move.damagingMove?
      ...
      when :HarshSun
        if move.calcType == :WATER
          @battle.pbDisplay(_INTL("¡El sol brilla con tanta intensidad que los movimientos de tipo Agua no tienen efecto!"))
          user.lastMoveFailed = true
          pbCancelMoves
          pbEndTurn(choice)
          return
    ...
    if targets.length == 0 && move.pbTarget(user).num_targets > 0 && !move.worksWithNoTargets?
      ...
      if numHits > 1
        ...
        if realNumHits == 1
          ...
          @battle.pbDisplay(_INTL("N.º de golpes: {1}.", realNumHits))
        end
      end
