...
class PokemonSystem
  ...
  attr_accessor :translate_moves
  attr_accessor :translate_abilities
  attr_accessor :translate_natures
  attr_accessor :coop_wild_invites
  attr_accessor :coop_trainer_invites
  ...
  def initialize
    ...
    @translate_moves = 0
    @translate_abilities = 0
    @translate_natures = 0
    @coop_wild_invites = 0 # 0=On, 1=Off
    @coop_trainer_invites = 0 # 0=On, 1=Off
  end
...
MenuHandlers.add(:options_menu, :translate_moves, {
  "name"        => _INTL("Traduzir Ataques"),
  "order"       => 81,
  "type"        => EnumOption,
  "parameters"  => [_INTL("Ligar"), _INTL("Desligar")],
  "description" => _INTL("Alterna os nomes dos ataques entre PT-BR e inglês."),
  "get_proc"    => proc { next $PokemonSystem.translate_moves || 0 },
  "set_proc"    => proc { |value, _scene|
    current = $PokemonSystem.translate_moves || 0
    next if current == value
    $PokemonSystem.translate_moves = value
    PTBR_TEXT.clear_runtime_cache! if defined?(PTBR_TEXT)
  }
})

MenuHandlers.add(:options_menu, :translate_abilities, {
  "name"        => _INTL("Traduzir Habilidades"),
  "order"       => 82,
  "type"        => EnumOption,
  "parameters"  => [_INTL("Ligar"), _INTL("Desligar")],
  "description" => _INTL("Alterna os nomes das habilidades entre PT-BR e inglês."),
  "get_proc"    => proc { next $PokemonSystem.translate_abilities || 0 },
  "set_proc"    => proc { |value, _scene|
    current = $PokemonSystem.translate_abilities || 0
    next if current == value
    $PokemonSystem.translate_abilities = value
    PTBR_TEXT.clear_runtime_cache! if defined?(PTBR_TEXT)
  }
})

MenuHandlers.add(:options_menu, :translate_natures, {
  "name"        => _INTL("Traduzir Natures"),
  "order"       => 83,
  "type"        => EnumOption,
  "parameters"  => [_INTL("Ligar"), _INTL("Desligar")],
  "description" => _INTL("Alterna os nomes das natures entre PT-BR e inglês."),
  "get_proc"    => proc { next $PokemonSystem.translate_natures || 0 },
  "set_proc"    => proc { |value, _scene|
    current = $PokemonSystem.translate_natures || 0
    next if current == value
    $PokemonSystem.translate_natures = value
    PTBR_TEXT.clear_runtime_cache! if defined?(PTBR_TEXT)
  }
})

MenuHandlers.add(:options_menu, :coop_wild_invites, {
  "name"        => _INTL("Convites Selvagens"),
  "order"       => 84,
  "type"        => EnumOption,
  "parameters"  => [_INTL("Ligado"), _INTL("Desligado")],
  "description" => _INTL("Escolha se deseja receber convites de coop em batalhas selvagens."),
  "get_proc"    => proc { next $PokemonSystem.coop_wild_invites || 0 },
  "set_proc"    => proc { |value, _scene| $PokemonSystem.coop_wild_invites = value }
})

MenuHandlers.add(:options_menu, :coop_trainer_invites, {
  "name"        => _INTL("Convites NPC"),
  "order"       => 85,
  "type"        => EnumOption,
  "parameters"  => [_INTL("Ligado"), _INTL("Desligado")],
  "description" => _INTL("Escolha se deseja receber convites de coop em batalhas contra NPCs."),
  "get_proc"    => proc { next $PokemonSystem.coop_trainer_invites || 0 },
  "set_proc"    => proc { |value, _scene| $PokemonSystem.coop_trainer_invites = value }
})
