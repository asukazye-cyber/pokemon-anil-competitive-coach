if Settings::RESTORE_HELD_ITEMS_AFTER_BATTLE
  class Battle::Battler
    ...
    alias pbRemoveItem_restore pbRemoveItem
    def pbRemoveItem(permanent = true)
      @battle.used_items << [self.pokemon, @item_id] if @battle.used_items && pbOwnedByPlayer? && permanent
      pbRemoveItem_restore(permanent)
    end
  end
