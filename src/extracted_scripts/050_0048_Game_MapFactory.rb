...
class PokemonMapFactory
  ...
  def map
    ...
    raise "No hay mapas en el archivo guardado... (mapIndex=#{@mapIndex})" if @maps.length == 0
    if @maps[0]
      echoln "Usando el próximo mapa, puede ser incorrecto (mapIndex=#{@mapIndex}, longitud=#{@maps.length})"
      return @maps[0]
    end
    raise "No hay mapas en el archivo guardado... (todos los mapas están vacíos; mapIndex=#{@mapIndex})"
  end
