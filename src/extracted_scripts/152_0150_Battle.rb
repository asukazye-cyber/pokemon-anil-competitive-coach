...
class Battle
  ...
  def pbReviveInParty(idxBattler, canCancel = false)
    ...
    pkmn.heal_status
    displayname = (pbOwnedByPlayer?(idxBattler)) ? pkmn.name.force_encoding("UTF-8") : _INTL("El {1} enemigo", pkmn.name.force_encoding("UTF-8"))
  ...
  def pbShowAbilitySplash(battler, delay = false, logTrigger = true)
    ...
    if delay
      ...
      until System.real_uptime - timer_start >= 1   # 1 second
        @scene.pbUpdate
      end
    end
  end
  ...
  def pbCalcOptimalType(user, target, move, effective_types, target_types, check_type)
    ...
    # DESSINCRONIZACAO ONLINE: .sample usa o RNG GLOBAL do Ruby, que nao e
    # sincronizado entre os dois clientes. Como este metodo roda na EXECUCAO do
    # golpe (Judgment/Legend Plate -> pbGetBestTypeJudgment), cada lado podia
    # sortear um TIPO diferente entre os igualmente efetivos -> dano e
    # efetividade divergentes -> desync. pbRandom passa pelo RNG semeado
    # compartilhado (ver 009_RNG_Patch do Cable Club).
    return effective_types[pbRandom(effective_types.length)]
  end
end