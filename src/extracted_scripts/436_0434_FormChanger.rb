...
def change_pokemon_form(force_species = nil, message_var = nil)
  ...
  if form_index != -1 && form_index < forms.length
    ...
    pokemon.forced_ability = nil if defined?(pokemon.forced_ability)
    pokemon.calc_stats
    pokemon.reset_moves
    text = _INTL("¡Listo! ¡He cambiado la forma de {1} a {2}!", pokemon.name, form_names[form_index])
