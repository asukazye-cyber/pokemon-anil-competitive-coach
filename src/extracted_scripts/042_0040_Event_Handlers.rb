...
class HandlerHashEnum
  ...
  def add(sym, handler = nil, &handlerBlock)
    if ![Proc, Hash].include?(handler.class) && !block_given?
      raise ArgumentError, "#{self.class.name} para #{sym.inspect} no tiene un controlador (handler) válido (se proporcionó #{handler.inspect})"
    end
    id = fromSymbol(sym)
  ...
  def addIf(conditionProc, handler = nil, &handlerBlock)
    if ![Proc, Hash].include?(handler.class) && !block_given?
      raise ArgumentError, "La llamada addIf para #{self.class.name} no tiene un controlador (handler) válido (se proporcionó #{handler.inspect})"
    end
