...
def pbGenerateChallenge(rule, tag)
  oldrule = rule
  yield(_INTL("Preparándose para generar los equipos"))
  ...
  iterations.times do |iter|
    ...
    yield(_INTL("Simulando combates ({1} de {2})", iter + 1, iterations))
    i = 0
    loop do
      changed = false
...
def pbWriteCup(id, rules)
  ...
  cmd = 0
  if trlists.length == 0
    cmd = pbMessage(_INTL("¿Generar equipos de Pokémon para este reto?"),
                    [_INTL("SÍ"), _INTL("NO")], 2)
    case cmd
    when 0
      cmd = 2
    ...
    cmd = pbMessage(_INTL("¿Generar equipos de Pokémon para este reto?"),
                    [_INTL("NO"), _INTL("SÍ, USAR EXISTENTE"), _INTL("SÍ, USAR NUEVO")], 1)
  end
  return if cmd == 0   # No
  case cmd
  when 1   # Yes, use existing
    cmd = pbMessage(_INTL("Elige un reto."), list, -1)
    if cmd >= 0
      pbMessage(_INTL("Este reto usará la lista de Pokémon de {1}.", list[cmd]))
      trlists.length.times do |i|
        tr = trlists[i]
    ...
    return
  when 2   # Yes, use new
    return if !pbConfirmMessage(_INTL("Esto puede llevar un buen rato. ¿Estás seguro?"))
