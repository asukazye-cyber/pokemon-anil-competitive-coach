...
class PokemonStorageScene
  ...
  message = PTBR_TEXT.t(message) if message.is_a?(String)
  ...
  def pbSelectBoxInternal(_party)
    ...
    loop do
      ...
      if Input.trigger?(Input::JUMPUP)
        ...
        pbUpdateOverlay(selection)
        pbSetMosaic(selection)
      elsif Input.trigger?(Input::AUX2)   # Jump to box name
        if selection != -1
          pbPlayCursorSE
          selection = -1
          pbSetArrow(@sprites["arrow"], selection)
  ...
  def gray_out_fainted_pokemon
    do_greys(proc { |poke| !poke.perma_faint })
  end
...
class PokemonStorageScreen
  ...
  def organise_commands(selected, pokemon)
    ...
    if heldpoke
      ...
      helptext = _INTL("Has seleccionado a {1}.", @heldpkmn.name)
    else
      helptext = _INTL("No has seleccionado ningún Pokémon.")
    end
