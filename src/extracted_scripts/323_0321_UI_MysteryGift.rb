...
#===============================================================================
module MysteryGift
  URL = "https://pastebin.com/raw/t6tSp4P5"
end
...
def pbManageMysteryGifts
  ...
  master_raw = IO.read("MysteryGiftMaster.txt")
  master = pbMysteryGiftDecrypt(master_raw)
  ...
  # Detectar si el formato del archivo necesita actualizarse
  begin
    expected_encrypted = Settings::ENCRIPTAR_REGALOS_MISTERIOSOS_EN_MASTER
    current_encrypted = false
    
    # Detectar si el archivo actual está encriptado
    begin
      Marshal.load(Zlib::Inflate.inflate(master_raw.unpack("m")[0]))
      current_encrypted = true
    rescue
      current_encrypted = false
    end
    
    # Si el formato no coincide con la configuración, actualizar el archivo
    if current_encrypted != expected_encrypted
      string = pbMysteryGiftEncrypt(master)
      File.open("MysteryGiftMaster.txt", "wb") { |f| f.write(string) }
      formato_nuevo = expected_encrypted ? "encriptado" : "texto plano"
      pbMessage(_INTL("El archivo \"MysteryGiftMaster.txt\" se ha convertido a formato {1}.", formato_nuevo))
    end
  rescue => e
    Console.echo_warn("No se pudo actualizar el formato del archivo: #{e.message}")
  end
...
def pbDownloadMysteryGift(trainer)
  ...
  loop do
    ...
    command = pbMessage(_INTL("Elige qué tipo de Regalo Misterioso descargar."), [
...
def pbMysteryGiftEncrypt(gift, master = true)
  gift.each do |gft|
    if gft[2].is_a?(Pokemon)
      gft[2] = crear_hash_a_partir_de_pokemon(gft[2])
    end
  end
  ...
  if Settings::ENCRIPTAR_REGALOS_MISTERIOSOS_EN_MASTER || !master
    ...
    ret = "[\n"
    gift.each_with_index do |gft, idx|
      ret += "  #{gft.inspect}"
      ret += ",\n" if idx < gift.length - 1
      ret += "\n" if idx == gift.length - 1
    end
    ret += "]"
  end
...
def pbMysteryGiftDecrypt(gift, master = true)
  ...
  is_encrypted = false
  begin
    # Intentar decodificar como base64 y descomprimir
    Marshal.load(Zlib::Inflate.inflate(gift.unpack("m")[0]))
    is_encrypted = true
  rescue
    is_encrypted = false
  end
  ...
  begin
    ...
    Console.echo_error("Error al desencriptar regalo misterioso: #{e.message}")
    return []
  end
  ...
  if ret
    ret.each do |gft|
      if gft[1] == 0   # Pokémon
        # Cargamos el Pokémon desde el hash
        gft[2] = crear_pokemon_a_partir_de_hash(gft[2])
      else   # Item
...
#===============================================================================
# CREACION DE HASH A PARTIR DE UN POKEMON
#===============================================================================

def crear_hash_a_partir_de_pokemon(pokemon)
...
def crear_pokemon_a_partir_de_hash(poke_hash)
  # Si ya es un Pokemon, devolverlo directamente
  return poke_hash if poke_hash.is_a?(Pokemon)
  
  # Si es un hash, crear el Pokemon
