...
module ItemBallPrinter
  ...
  def self.get_item_ball_string(map_id, mapinfos=nil, type = :item_balls)
    ...
    if type == :mart_items
      event_and_scripts = self.get_events_and_mart_scripts(map.events.values)
    else
    ...
    for i in 0...event_and_scripts.size
      ...
      if type == :mart_items
        items = self.extract_mart_items(script)
        line = sprintf(
          "%03d. ID: %03d (%03d,%03d) items: %s  event: %s", 
          i+1, event.id, event.x, event.y, items, event.name
        )
      else
        item = script.gsub("pbItemBall","").gsub("pbReceiveItem","").gsub("(","").gsub(")","").gsub(":","").chomp
        line = sprintf(
  ...
  # Return an array of arrays with two items: the valid event, script text with mart
  def self.get_events_and_mart_scripts(events)
    ret = []
    event_name = "pbPokemonMart"
  ...
  # Extract items from pbPokemonMart script
  def self.extract_mart_items(script)
    # Match the array part inside pbPokemonMart([...])
    match = script.match(/pbPokemonMart\s*\(\s*\[(.*?)\]/m)
    return "" if !match
    
    items_text = match[1]
    # Split by commas and clean up each item
    items = items_text.split(',').map do |item|
      item.strip.gsub(/^:/, '').gsub(/[\s\n\r]/, '')
    end.reject(&:empty?)
...
MenuHandlers.add(:debug_menu, :item_ball_print_current_map, {
  ...
  "effect"      => proc{
    ...
    mart_items = pbConfirmMessage(_INTL("¿Quieres imprimir tambien las tiendas? (pbPokemonMart)"))
    ItemBallPrinter.print_current_map(:item_balls)
    ItemBallPrinter.print_current_map(:receive_items) if receive_items
    ItemBallPrinter.print_current_map(:mart_items) if mart_items
    pbMessage(_INTL("¡Impreso!"))}
})
...
MenuHandlers.add(:debug_menu, :item_ball_print_all, {
  ...
  "effect"      => proc{
    ...
    ItemBallPrinter.generate_txt(:mart_items) if mart_items
...
MenuHandlers.add(:debug_menu, :item_ball_print_mart_items, {
  "parent"      => :item_ball_printer,
  "name"        => _INTL("Imprimir solo tiendas"),
  "description" => _INTL("Imprime solo las tiendas (pbPokemonMart) en {1}.", ItemBallPrinter.file_full_name(:mart_items)),
  "effect"      => proc{
    msgwindow = pbCreateMessageWindow
    if FileTest.exist?(ItemBallPrinter.file_full_name(:mart_items)) && !pbConfirmMessageSerious(
       _INTL("{1} ya existe. ¿Quieres sobrescribirlo?",ItemBallPrinter.file_full_name(:mart_items))
    )
      pbDisposeMessageWindow(msgwindow)
      next
    end
    pbMessageDisplay(msgwindow,_INTL("Por favor, espera.\\wtnp[0]"))
    ItemBallPrinter.generate_txt(:mart_items)
    pbMessageDisplay(msgwindow,_INTL("¡Impreso!"))
    pbDisposeMessageWindow(msgwindow)
  }
})