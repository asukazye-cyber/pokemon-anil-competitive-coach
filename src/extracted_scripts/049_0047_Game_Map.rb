...
class Game_Map
  ...
  def setup(map_id)
    ...
    begin
      ...
      raise "Error loading Map #{map_id}: #{e.message}"
    end
  ...
  # Nem todo objeto em events e um Game_Event puro do RPG Maker: os mods de
  # multiplayer injetam eventos proprios no mapa (Pokemon remoto, item dropado,
  # Pokemon solto). Se algum deles chegar com tile_id nil, o "tile_id <= 0"
  # original estoura com "undefined method '<' for nil:NilClass" e derruba o
  # jogo no meio do andar. Trata-se nil como 0 (sem tile grafico), que e o
  # comportamento correto para evento de personagem.
  #
  # O log existe para achar o CULPADO: nil aqui e sempre bug de quem criou o
  # evento, e sem isso nao da para saber qual mod foi.
  # Grava SEMPRE, em arquivo proprio. Nao usa AnilLanRework.log de proposito:
  # aquele so escreve com $anil_debug_log_enabled ligado (padrao false), e este
  # aviso e justamente o que precisa sobreviver para achar o mod culpado.
  # Uma linha por assinatura, para nao encher o disco num loop de movimento.
  def anil_avisar_evento_quebrado(event, motivo)
    @anil_eventos_avisados ||= {}
    chave = "#{event.class}/#{motivo}"
    return if @anil_eventos_avisados[chave]
    @anil_eventos_avisados[chave] = true
    linha = "[#{Time.now.strftime("%Y-%m-%d %H:%M:%S")}] mapa=#{@map_id} classe=#{event.class} " \
            "id=#{(event.id rescue "?")} nome=#{(event.name rescue "?").inspect} " \
            "x=#{(event.x rescue "?").inspect} y=#{(event.y rescue "?").inspect} " \
            "tile_id=#{(event.tile_id rescue "?").inspect} -> #{motivo}"
    File.open("anil_eventos_quebrados.txt", "a:UTF-8") { |f| f.puts(linha) }
    AnilLanRework.log("[GAME_MAP] #{linha}") rescue nil
  rescue
  end

  # tile_id de um evento, com nil tratado como 0 (evento de personagem nao tem
  # tile grafico). Evento injetado por mod as vezes chega sem esse campo.
  def anil_tile_id_de(event)
    tid = (event.tile_id rescue nil)
    return tid if tid.is_a?(Integer)
    anil_avisar_evento_quebrado(event, "tile_id nil (tratado como 0)")
    0
  end

  # O at_coordinate? do Game_Character compara check_x >= @x. Um evento sem
  # coordenada (x/y nil) estoura ali com "comparison of Integer with nil failed"
  # e derruba o jogo enquanto o jogador anda. Evento sem posicao nao ocupa tile
  # nenhum, entao a resposta correta e simplesmente false.
  def anil_evento_ocupa?(event, x, y)
    event.at_coordinate?(x, y)
  rescue => e
    anil_avisar_evento_quebrado(event, "at_coordinate? falhou (#{e.class})")
    false
  end
  ...
  def passable?(x, y, d, self_event = nil)
    ...
    events.each_value do |event|
      event_tile_id = anil_tile_id_de(event)
      next if event_tile_id <= 0
      next if event == self_event
      next if !anil_evento_ocupa?(event, x, y)
      next if event.through
      next if GameData::TerrainTag.try_get(@terrain_tags[event_tile_id]).ignore_passability
      passage = @passages[event_tile_id]
      return false if passage & bit != 0
      return false if passage & 0x0f == 0x0f
      return true if @priorities[event_tile_id] == 0
    end
  ...
  def passableStrict?(x, y, d, self_event = nil)
    return false if !valid?(x, y)
    events.each_value do |event|
      # Ver anil_tile_id_de: evento injetado por mod pode vir com tile_id nil, e
      # o "< 0" original derrubava o jogo ao andar (NoMethodError '<' for nil).
      event_tile_id = anil_tile_id_de(event)
      next if event == self_event || event_tile_id < 0 || event.through
      next if !anil_evento_ocupa?(event, x, y)
      next if GameData::TerrainTag.try_get(@terrain_tags[event_tile_id]).ignore_passability
      return false if @passages[event_tile_id] & 0x0f != 0
      return true if @priorities[event_tile_id] == 0
    end
    [2, 1, 0].each do |i|
      tile_id = data[x, y, i]
